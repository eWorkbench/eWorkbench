...
class DSSFileWatch:
    ...
    def _connect(self, connection_parameters):
        try:
            ...
            LOGGER.info('Connected to {}'.format(self.queue))
    ...
    def api_login(self):
        LOGGER.info('Login at {}'.format(self.api_login_endpoint))

        payload = {"username": self.api_user, "password": self.api_pass}
        token_request = requests.post(
            url=self.api_login_endpoint,
            json=payload
        )
    ...
    def post_files(self, token):
        while True:
            ...
            if message_count > 0:
                ...
                try:
                    LOGGER.info('Notify {} about {} new files'.format(
                        self.api_endpoint,
                        self.message_fetch_size
                    ))
