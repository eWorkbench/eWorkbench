...
from eric.projects.models import Group, Project, Resource, Role, User
from eric.projects.tests.core import AuthenticationMixin, ModelPrivilegeMixin, ProjectsMixin, ResourceMixin
...
class TestProjectMembers(
    ...
    def setUp(self):
        ...
        self.dmp_form_field = DmpFormField.objects.create(
            name="dmpformfield1", type="NUM", infotext="unittest dmpformfield1", dmp_form=self.dmp_form
        )
        ...
        self.data = [
            ...
            {"subject": "Test Note", "content": "<p>Some <b>Note</b> Content</p>", "project_pks": self.project1.pk},
            {
                "title": "Test Meeting",
            ...
            {"title": "Test LabBook", "is_template": False, "project_pks": self.project1.pk},
            {"title": "Test Drive", "project_pks": self.project1.pk},
            {
                "dmp_title": "Test DMP",
                "dmp_status": Dmp.NEW,
            ...
            {"title": "Test Picture", "background_img_file_name": "demo1.png", "project_pks": self.project1.pk},
            {"title": "Test KanbanBoard", "project_pks": self.project1.pk},
        ]
    ...
    def test_task_privileges_for_project_members(self):
        ...
        self.assertEqual(len(decoded_list), 1, msg="User 2 should see 1 task")
        ...
        self.assertEqual(len(decoded_privileges), 2, msg="There should be two privileges for this task")
    ...
    def test_note_privileges_for_project_members(self):
        ...
        response = self.rest_create_note(
            self.token1, HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR, **self.data[1]
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        decoded_response = json.loads(response.content.decode())
        ...
        decoded_list = json.loads(response.content.decode())
        decoded_list = test_utils.get_paginated_results(decoded_list)
        self.assertEqual(len(decoded_list), 1, msg="User 2 should see 1 note")

        # and there should be a view, edit, trash and restore privilege listed for user 2
        response = self.rest_get_privileges(self.token1, "notes", note.pk, HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        decoded_privileges = json.loads(response.content.decode())
        self.assertEqual(len(decoded_privileges), 2, msg="There should be two privileges for this note")
        # privilege 0 should be for user1
        self.assertEqual(decoded_privileges[0]["user"]["pk"], self.user1.pk)
        # and privilege 1 should be for user2
        self.assertEqual(decoded_privileges[1]["user"]["pk"], self.user2.pk)
    ...
    def test_meeting_privileges_for_project_members(self):
        ...
        response = self.rest_create_meeting(
            self.token1, HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR, **self.data[2]
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        decoded_response = json.loads(response.content.decode())
    ...
    def test_file_privileges_for_project_members(self):
        ...
        response = self.rest_create_file(
            self.token1, HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR, **self.data[3]
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        decoded_response = json.loads(response.content.decode())
        ...
        decoded_list = json.loads(response.content.decode())
        decoded_list = test_utils.get_paginated_results(decoded_list)
        self.assertEqual(len(decoded_list), 1, msg="User 2 should see 1 file")

        # and there should be a view, edit, trash and restore privilege listed for user 2
        response = self.rest_get_privileges(self.token1, "files", file.pk, HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        decoded_privileges = json.loads(response.content.decode())
        self.assertEqual(len(decoded_privileges), 2, msg="There should be two privileges for this file")
        # privilege 0 should be for user1
        self.assertEqual(decoded_privileges[0]["user"]["pk"], self.user1.pk)
        # and privilege 1 should be for user2
        self.assertEqual(decoded_privileges[1]["user"]["pk"], self.user2.pk)
    ...
    def test_contact_privileges_for_project_members(self):
        ...
        response = self.rest_create_contact(
            self.token1, HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR, **self.data[4]
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        decoded_response = json.loads(response.content.decode())
        ...
        decoded_list = json.loads(response.content.decode())
        decoded_list = test_utils.get_paginated_results(decoded_list)
        self.assertEqual(len(decoded_list), 1, msg="User 2 should see 1 contact")

        # and there should be a view, edit, trash and restore privilege listed for user 2
        response = self.rest_get_privileges(self.token1, "contacts", contact.pk, HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        decoded_privileges = json.loads(response.content.decode())
        self.assertEqual(len(decoded_privileges), 2, msg="There should be two privileges for this contact")
        # privilege 0 should be for user1
        self.assertEqual(decoded_privileges[0]["user"]["pk"], self.user1.pk)
        # and privilege 1 should be for user2
        self.assertEqual(decoded_privileges[1]["user"]["pk"], self.user2.pk)
    ...
    def test_labbook_privileges_for_project_members(self):
        ...
        response = self.rest_create_labbook(
            self.token1, HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR, **self.data[5]
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        decoded_response = json.loads(response.content.decode())
        ...
        decoded_list = json.loads(response.content.decode())
        decoded_list = test_utils.get_paginated_results(decoded_list)
        self.assertEqual(len(decoded_list), 1, msg="User 2 should see 1 labbook")

        # and there should be a view, edit, trash and restore privilege listed for user 2
        response = self.rest_get_privileges(self.token1, "labbooks", labbook.pk, HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        decoded_privileges = json.loads(response.content.decode())
        self.assertEqual(len(decoded_privileges), 2, msg="There should be two privileges for this labbook")
        # privilege 0 should be for user1
        self.assertEqual(decoded_privileges[0]["user"]["pk"], self.user1.pk)
        # and privilege 1 should be for user2
        self.assertEqual(decoded_privileges[1]["user"]["pk"], self.user2.pk)
    ...
    def test_drive_privileges_for_project_members(self):
        ...
        response = self.rest_create_drive(
            self.token1, HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR, **self.data[6]
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        decoded_response = json.loads(response.content.decode())
        ...
        decoded_list = json.loads(response.content.decode())
        decoded_list = test_utils.get_paginated_results(decoded_list)
        self.assertEqual(len(decoded_list), 1, msg="User 2 should see 1 drive")

        # and there should be a view, edit, trash and restore privilege listed for user 2
        response = self.rest_get_privileges(self.token1, "drives", drive.pk, HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        decoded_privileges = json.loads(response.content.decode())
        self.assertEqual(len(decoded_privileges), 2, msg="There should be two privileges for this drive")
        # privilege 0 should be for user1
        self.assertEqual(decoded_privileges[0]["user"]["pk"], self.user1.pk)
        # and privilege 1 should be for user2
        self.assertEqual(decoded_privileges[1]["user"]["pk"], self.user2.pk)
    ...
    def test_dmp_privileges_for_project_members(self):
        ...
        response = self.rest_create_dmp(
            self.token1, HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR, **self.data[7]
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        decoded_response = json.loads(response.content.decode())
        ...
        decoded_list = json.loads(response.content.decode())
        decoded_list = test_utils.get_paginated_results(decoded_list)
        self.assertEqual(len(decoded_list), 1, msg="User 2 should see 1 dmp")

        # and there should be a view, edit, trash and restore privilege listed for user 2
        response = self.rest_get_privileges(self.token1, "dmps", dmp.pk, HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        decoded_privileges = json.loads(response.content.decode())
        self.assertEqual(len(decoded_privileges), 2, msg="There should be two privileges for this dmp")
        # privilege 0 should be for user1
        self.assertEqual(decoded_privileges[0]["user"]["pk"], self.user1.pk)
        # and privilege 1 should be for user2
        self.assertEqual(decoded_privileges[1]["user"]["pk"], self.user2.pk)
    ...
    def test_resource_privileges_for_project_members(self):
        ...
        response = self.rest_create_resource(
            self.token1, HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR, **self.data[8]
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        decoded_response = json.loads(response.content.decode())
        ...
        decoded_list = json.loads(response.content.decode())
        decoded_list = test_utils.get_paginated_results(decoded_list)
        self.assertEqual(len(decoded_list), 1, msg="User 2 should see 1 resource")

        # and there should be a view, edit, trash and restore privilege listed for user 2
        response = self.rest_get_privileges(self.token1, "resources", resource.pk, HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        decoded_privileges = json.loads(response.content.decode())
        self.assertEqual(len(decoded_privileges), 2, msg="There should be two privileges for this resource")
        # privilege 0 should be for user1
        self.assertEqual(decoded_privileges[0]["user"]["pk"], self.user1.pk)
        # and privilege 1 should be for user2
        self.assertEqual(decoded_privileges[1]["user"]["pk"], self.user2.pk)
    ...
    def test_picture_privileges_for_project_members(self):
        ...
        response = self.rest_create_picture(
            self.token1, HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR, **self.data[9]
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        decoded_response = json.loads(response.content.decode())
        ...
        decoded_list = json.loads(response.content.decode())
        decoded_list = test_utils.get_paginated_results(decoded_list)
        self.assertEqual(len(decoded_list), 1, msg="User 2 should see 1 picture")

        # and there should be a view, edit, trash and restore privilege listed for user 2
        response = self.rest_get_privileges(self.token1, "pictures", picture.pk, HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        decoded_privileges = json.loads(response.content.decode())
        self.assertEqual(len(decoded_privileges), 2, msg="There should be two privileges for this picture")
        # privilege 0 should be for user1
        self.assertEqual(decoded_privileges[0]["user"]["pk"], self.user1.pk)
        # and privilege 1 should be for user2
        self.assertEqual(decoded_privileges[1]["user"]["pk"], self.user2.pk)
    ...
    def test_kanbanboard_privileges_for_project_members(self):
        ...
        response = self.rest_create_kanbanboard(
            self.token1, HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR, **self.data[10]
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        decoded_response = json.loads(response.content.decode())
        ...
        decoded_list = json.loads(response.content.decode())
        decoded_list = test_utils.get_paginated_results(decoded_list)
        self.assertEqual(len(decoded_list), 1, msg="User 2 should see 1 kanbanboard")

        # and there should be a view, edit, trash and restore privilege listed for user 2
        response = self.rest_get_privileges(self.token1, "kanbanboards", kanbanboard.pk, HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        decoded_privileges = json.loads(response.content.decode())
        self.assertEqual(len(decoded_privileges), 2, msg="There should be two privileges for this kanbanboard")
        # privilege 0 should be for user1
        self.assertEqual(decoded_privileges[0]["user"]["pk"], self.user1.pk)
        # and privilege 1 should be for user2
        self.assertEqual(decoded_privileges[1]["user"]["pk"], self.user2.pk)
