...
from eric.jwt_auth.jwt_utils import strip_url_params, add_url_params
...
class UtilsTestCase(TestCase):
    def test_strip_url_params(self):
        ...
        self._assert_strip_url_params_result(
            'workbench.local/some/directory/file?q=s=test%20test&q1=&q2=&flag&',
            'workbench.local/some/directory/file'
        )

        self._assert_strip_url_params_result(
            'workbench.local/some/directory/my-file',
            'workbench.local/some/directory/my-file'
        )
    ...
    def test_add_url_params(self):
        self._assert_add_url_params_result(
            'workbench.local', params={'q1': 'abc'},
            expected_result='workbench.local?q1=abc'
        )

        self._assert_add_url_params_result(
            'workbench.local', params={'q1': 'abc', 'q2': 'def'},
            expected_result='workbench.local?q1=abc&q2=def'
        )

        self._assert_add_url_params_result(
            'workbench.local?abc=def', params={'q1': 'abc', 'q2': 'def'},
            expected_result='workbench.local?abc=def&q1=abc&q2=def'
        )

        self._assert_add_url_params_result(
            'workbench.local', params={},
            expected_result='workbench.local'
        )

        self._assert_add_url_params_result(
            'workbench.local?q=x', params={},
            expected_result='workbench.local?q=x'
        )
        ...
        self._assert_add_url_params_result(
            '?q=1&p=2', params={},
            expected_result='?q=1&p=2'
        )

        self._assert_add_url_params_result(
            '?q=a&p=2', params={'q': 'b'},
            expected_result='?q=a&p=2&q=b'
        )
