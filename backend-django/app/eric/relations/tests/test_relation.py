...
from eric.shared_elements.tests.core import CommentMixin, ContactMixin, MeetingMixin, NoteMixin, TaskMixin
...
class RelationsTest(
    ...
    def setUp(self):
        ...
        self.user3 = User.objects.create_user(username="student_3", email="student_3@email.com", password="qwerty")
        ...
        self.task1, response = self.create_task_orm(
            self.token1,
            self.project1.pk,
            "The new Task",
            "Task Description",
        ...
        # create another task
        self.task2, response = self.create_task_orm(
            self.token1,
            self.project1.pk,
            "Another new Task",
            "Task Description",
        ...
        self.task3, response = self.create_task_orm(
            self.token1,
            None,
            "Another new Task without project",
            "Task Description",
            Task.TASK_STATE_NEW,
            Task.TASK_PRIORITY_VERY_HIGH,
            timezone.now(),
        ...
        self.note1, response = self.create_note_orm(
            self.token1, self.project1.pk, "Note1 Title", "Some Note Description", HTTP_USER_AGENT, REMOTE_ADDR
        )

        # create another note in a project
        self.note2, response = self.create_note_orm(
            self.token1, self.project1.pk, "Note2 Title", "Another Note Description", HTTP_USER_AGENT, REMOTE_ADDR
        )

        # create another note in a project
        self.note3, response = self.create_note_orm(
            self.token1, self.project1.pk, "Note3 Title", "Another Note Description", HTTP_USER_AGENT, REMOTE_ADDR
        )

        # create a note without a project
        self.note4, response = self.create_note_orm(
            self.token1, None, "Note4 Title without project", "Another Note Description", HTTP_USER_AGENT, REMOTE_ADDR
        )

        # create another note without a project
        self.note5, response = self.create_note_orm(
            self.token1, None, "Note5 Title without project", "Another Note Description", HTTP_USER_AGENT, REMOTE_ADDR
        )

        # create a meeting via API
        self.meeting1, response = self.create_meeting_orm(
            self.token1,
            self.project1.pk,
            "The new meeting",
        ...
        self.meeting2, response = self.create_meeting_orm(
            self.token1,
            self.project1.pk,
            "Another new meeting",
            "a meeting description",
        ...
        self.meeting3, response = self.create_meeting_orm(
            self.token1,
            None,
        ...
        self.contact1, response = self.create_contact_orm(
            self.token1, self.project1.pk, "Dr.", "Max", "Mustermann", "max@mustermann.at", HTTP_USER_AGENT, REMOTE_ADDR
        )

        # another new contact
        self.contact2, response = self.create_contact_orm(
            self.token1, self.project1.pk, "DI", "Max", "Musterfrau", "max@musterfrau.at", HTTP_USER_AGENT, REMOTE_ADDR
        )

        # another new contact without a project
        self.contact3, response = self.create_contact_orm(
            self.token1, None, "Dr.", "Maxine", "Musterfrau", "maxine@musterfrau.at", HTTP_USER_AGENT, REMOTE_ADDR
        )
    ...
    def test_create_self_linked_relation(self):
        ...
        self.assertEqual(
            ",".join(decoded["left_content_object"]),
    ...
    def test_create_relation(self):
        ...
        relation = self.create_task_relation(
            self.token1,
            self.task1.pk,
            Task.get_content_type(),
            self.task1.pk,
            Note.get_content_type(),
            self.note1.pk,
            False,
            HTTP_USER_AGENT,
            REMOTE_ADDR,
        ...
        self.assertEqual(len(decoded_response), 1, msg="Should be exactly one relation for this note")
        self.assertEqual(decoded_response[0]["pk"], str(relation.pk))
        ...
        self.assertEqual(len(decoded_response), 1, msg="Should be exactly one relation for this task")
        self.assertEqual(decoded_response[0]["pk"], str(relation.pk))
        ...
        decoded_response = test_utils.get_paginated_results(decoded_response)
        # should be exactly one
        self.assertEqual(len(decoded_response), 1, msg="Should be exactly one relation")
        self.assertEqual(decoded_response[0]["pk"], str(relation.pk))
        ...
        decoded_response = test_utils.get_paginated_results(decoded_response)
        # should be five relations
        self.assertEqual(len(decoded_response), 2, msg="Should be exactly two relations for this task")
        self.assertEqual(Relation.objects.all().count(), 2, msg="There should be exactly two relations")
        ...
        decoded_response = test_utils.get_paginated_results(decoded_response)
        # should be exactly one
        self.assertEqual(len(decoded_response), 1, msg="Should be exactly one relation for contact")
        self.assertEqual(decoded_response[0]["pk"], str(relation.pk))
        ...
        self.assertEqual(len(decoded_response), 3, msg="Should be exactly three relations for this task")
        self.assertEqual(Relation.objects.all().count(), 3, msg="There should be exactly three relations")
        ...
        decoded_response = test_utils.get_paginated_results(decoded_response)
        # should be five relations
        self.assertEqual(len(decoded_response), 4, msg="Should be exactly four relations for tasks")

        self.assertEqual(Relation.objects.all().count(), 4, msg="There should be exactly four relations")

    def test_create_relation_with_wrong_parameter(self):
        """Tries to create a relation with wrong parameters"""
        self.assertEqual(Relation.objects.all().count(), 0, msg="There should be zero relations to start with")

        # do not send the private field (allowed, as it is set to False per default)
        response = self.client.post(
            f"/api/tasks/{self.task1.pk}/relations/",
            {
        ...
        decoded = json.loads(response.content.decode())
        self.assertEqual(decoded["private"], False)
        ...
        self.assertEqual(Relation.objects.all().count(), 2, msg="There should be two relations")
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.content.decode(), '{"left_content_type":["This field is required."]}')
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.content.decode(), '{"left_content_type":["This field may not be null."]}')
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.content.decode(), '{"left_object_id":["This field is required."]}')
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.content.decode(), '{"left_content_object":["can not be none"]}')
    ...
    def test_relation_update_with_wrong_parameter(self):
        ...
        response = self.client.put(
            f"/api/tasks/{self.task1.pk}/relations/{relation.pk}/",
            {
        ...
        self.assertEqual(decoded_response["left_content_type"], int(Task.get_content_type().id))
    ...
    def test_relation_update_private_field(self):
        ...
        response = self.rest_update_task_relation(
            self.token1,
            self.task1.pk,
            relation.pk,
            Task.get_content_type(),
        ...
        decoded_response = json.loads(response.content.decode())
        self.assertEqual(decoded_response["private"], True)

        # get call with student 2 to receive the private relation (not allowed)
        response = self.rest_get_task_relation(self.token2, self.task1.pk, relation.pk, HTTP_USER_AGENT, REMOTE_ADDR)
        ...
        # decode response and load it into json
        decoded_response = json.loads(response.content.decode())
        self.assertEqual(decoded_response["pk"], str(relation.pk))
    ...
    def test_delete_relation(self):
        ...
        response = self.rest_delete_task_relation(self.token1, self.task1.pk, relation.pk, HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)

        # try to get the just deleted relation with student 1 (does not work obviously)
        response = self.rest_get_task_relation(self.token1, self.task1.pk, relation.pk, HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        ...
        response = self.rest_delete_task_relation(self.token2, self.task1.pk, relation.pk, HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertContains(
            response, _("You do not have permission to perform this action"), status_code=status.HTTP_403_FORBIDDEN
        )
    ...
    def test_relate_task_to_everything(self):
        ...
        self.assertEqual(len(decoded_content), 5, msg="There should be 5 relations for this task")
        self.assertEqual(Relation.objects.all().count(), 5, msg="There should be 5 relations")
    ...
    def test_relate_meeting_to_everything(self):
        ...
        decoded_content = json.loads(response.content.decode())
        decoded_content = test_utils.get_paginated_results(decoded_content)
        self.assertEqual(len(decoded_content), 5, msg="There should be 5 relations for this meeting")
        self.assertEqual(Relation.objects.all().count(), 5, msg="There should be 5 relations")
    ...
    def test_relate_contact_to_everything(self):
        ...
        decoded_content = json.loads(response.content.decode())
        decoded_content = test_utils.get_paginated_results(decoded_content)
        self.assertEqual(len(decoded_content), 5, msg="There should be 5 relations for this contact")
        self.assertEqual(Relation.objects.all().count(), 5, msg="There should be 5 relations")
    ...
    def test_relate_note_to_everything(self):
        ...
        decoded_content = json.loads(response.content.decode())
        decoded_content = test_utils.get_paginated_results(decoded_content)
        self.assertEqual(len(decoded_content), 5, msg="There should be 5 relations for this note")
        self.assertEqual(Relation.objects.all().count(), 5, msg="There should be 5 relations")
    ...
    def test_relations_can_not_be_created_if_not_viewable_by_user(self):
        ...
        self.assertContains(
            response, _("You do not have permission to relate"), status_code=status.HTTP_400_BAD_REQUEST
        )
        ...
        self.assertEqual(Relation.objects.all().count(), 0, msg="There should be zero relations")
    ...
    def test_relate_note_to_everything_view_permissions(self):
        ...
        self.assertEqual(len(decoded_response), 1, msg="There should be exactly one relation with the task")
        self.assertEqual(
            decoded_response[0]["left_object_id"],
            str(self.note4.pk),
            msg="Left object id of relation should be note4.pk",
        )
        self.assertEqual(
            decoded_response[0]["left_content_object"]["subject"],
            self.note4.subject,
            msg="Left content object should have the notes subject",
        )
        self.assertEqual(
            decoded_response[0]["left_content_object"]["content"],
            self.note4.content,
        ...
        # verify response to contain the note
        self.assertEqual(len(decoded_response), 1, msg="There should be exactly one relation with the meeting")
        self.assertEqual(
            decoded_response[0]["left_object_id"],
            str(self.note4.pk),
            msg="Left object id of relation should be note4.pk",
        ...
        self.assertEqual(len(decoded_response), 1, msg="There should be exactly one relation with the contact")
        self.assertEqual(
            decoded_response[0]["left_object_id"],
            str(self.note4.pk),
            msg="Left object id of relation should be note4.pk",
        ...
        # verify response to contain the note
        self.assertEqual(len(decoded_response), 1, msg="There should be exactly one relation with the note")
        self.assertEqual(
            decoded_response[0]["left_object_id"],
            str(self.note4.pk),
            msg="Left object id of relation should be note4.pk",
    ...
    def test_delete_related_object_should_also_delete_relation(self):
        ...
        self.assertEqual(Relation.objects.filter(pk=relation1.pk).exists(), False)
        self.assertEqual(Relation.objects.filter(pk=relation2.pk).exists(), False)

        # however, the personal note still exists
        self.assertEqual(Note.objects.filter(pk=personal_note.pk).exists(), True)
