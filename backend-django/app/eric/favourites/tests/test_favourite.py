...
class FavouritesTest(APITestCase, CommonTestMixin, AuthenticationMixin, ProjectsMixin, FavouritesMixin):
    def setUp(self):
        ...
        self.rest_assign_user_to_project(
            self.token1, self.project1, self.user2, self.observerRole,
    ...
    def test_create_favourite(self):
        ...
        self.assertEquals(len(decoded_response['results']), 1,
                          msg="There should be exactly one favourite for this project")
        self.assertEquals(decoded_response['results'][0]['pk'], str(self.project1.pk))
        ...
        self.assertEquals(len(decoded_response['results']), 2,
                          msg="There should be exactly two favourites for projects")
        self.assertEquals(Favourite.objects.all().count(), 2,
                          msg="There should be exactly two favourites")
    ...
    def test_create_favourite_and_verify_response(self):
        ...
        self.assertEqual(Favourite.objects.all().count(), initial_favourite_length + 1,
        ...
        self.assertEqual(favourite.content_type, content_type,
    ...
    def test_favourites_can_not_be_created_if_not_viewable_by_user(self):
        ...
        self.assertEquals(Favourite.objects.all().count(), 1, msg="There should be one favourite")
...
class FavouriteFilterTest(APITestCase, CommonTestMixin, AuthenticationMixin, ProjectsMixin, FavouritesMixin):
    def setUp(self):
        ...
        self.project1 = self.create_project(
            self.token,
            "Project 1", "Description", Project.INITIALIZED,
            HTTP_USER_AGENT, REMOTE_ADDR
        )

        self.project2 = self.create_project(
            self.token,
            "Project 2", "Description", Project.INITIALIZED,
            HTTP_USER_AGENT, REMOTE_ADDR
        )

        self.fav_project1 = self.create_project(
            self.token,
            "Fav Project 1", "Description", Project.INITIALIZED,
            HTTP_USER_AGENT, REMOTE_ADDR
        )
    ...
    def test_favourite_filter_empty(self):
        ...
        self.assertEquals(4, json_response['count'])
    ...
    def test_favourite_filter_true(self):
        ...
        json_response = self.parse_response(response)
        self.assertEquals(1, json_response['count'])
    ...
    def test_favourite_filter_false(self):
        ...
        json_response = self.parse_response(response)
        self.assertEquals(3, json_response['count'])
