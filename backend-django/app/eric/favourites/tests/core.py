...
class FavouritesMixin:
    ...
    def rest_get_favourite(
            self, auth_token, favourite_id,
    ...
    def rest_get_favourite_projects(
            self, auth_token,
            HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR
    ):
        self.client.credentials(HTTP_AUTHORIZATION='Token ' + auth_token)
        ...
        response = self.client.get(
            '/api/projects/?favourite=true',
            HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR
        )
        return response

    def rest_get_projects_with_favourite_param(
            self, auth_token, favourite,
            HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR
    ):
        self.client.credentials(HTTP_AUTHORIZATION='Token ' + auth_token)

        response = self.client.get(
            f'/api/projects/?favourite={favourite}',
            HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR
        )
        return response

    def rest_create_favourite(
            self, auth_token, object_id, content_type_pk,
            HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR
    ):
        self.client.credentials(HTTP_AUTHORIZATION='Token ' + auth_token)
    ...
    def rest_delete_favourite(
            self, auth_token, object_id, content_type,
            HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR
    ):
        self.client.credentials(HTTP_AUTHORIZATION='Token ' + auth_token)
