...
class TestLabBookSections(APITestCase, LabBookMixin, LabbookSectionMixin,
                          AuthenticationMixin, ProjectsMixin, FileMixin):
    ...
    def setUp(self):
        ...
        self.labbooksection_view_permission = Permission.objects.filter(
            codename='add_labbooksection',
            content_type=LabbookSection.get_content_type()
        ).first()
    ...
    def test_labbook_section_elements(self):
        ...
        labbooksection, response = self.create_labbooksection_orm(self.token1, self.project1.pk, self.dates['today'],
                                                                  'Section 1', [], **self.http_data)
        ...
        response = self.rest_add_labbook_element(
            self.token1, labbook.pk, labbooksection.get_content_type().id, labbooksection.pk, 0, 0, 20, 1, **self.http_data
        )
        ...
        self.assertEquals(decoded_response['height'], 1)
        self.assertEquals(decoded_response['child_object_content_type'], labbooksection.get_content_type().id)
        self.assertEquals(decoded_response['child_object_id'], str(labbooksection.pk))
        ...
        self.assertEquals(response.status_code, status.HTTP_200_OK)
        self.assertEquals(response.data['date'], self.dates['today'].isoformat())
        self.assertEquals(response.data['title'], 'Section 1')
        self.assertEquals(response.data['child_elements'], [])

        # get the section child elements, should be 0
        response = self.rest_get_labbook_elements_for_section(self.token1, labbook.pk, labbooksection.pk,
                                                              **self.http_data)
        self.assertEquals(response.status_code, status.HTTP_200_OK)
        decoded_list = json.loads(response.content.decode())
        self.assertEquals(len(decoded_list), 0)
        ...
        response = self.rest_update_labbooksection(self.token1, labbooksection.pk, self.project1.pk,
                                                   self.dates['today'], 'Section 1',
        ...
        decoded_list = json.loads(response.content.decode())
        self.assertEquals(len(decoded_list), 1)
        self.assertEquals(decoded_list[0]['child_object_content_type_model'], 'shared_elements.file')
        self.assertEquals(decoded_list[0]['child_object']['name'], 'test.txt')
        ...
        decoded_list = json.loads(response.content.decode())
        self.assertEquals(len(decoded_list), 1)
        self.assertEquals(decoded_list[0]['child_object_content_type_model'], 'labbooks.labbooksection')
        self.assertEquals(decoded_list[0]['child_object']['date'], self.dates['today'].isoformat())
        self.assertEquals(decoded_list[0]['child_object']['title'], 'Section 1')
        self.assertEquals(decoded_list[0]['child_object']['child_elements'], [labbook_child_element_file['pk']])
