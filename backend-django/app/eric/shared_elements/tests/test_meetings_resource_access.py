...
class ResourceAccessTest(APITestCase, AuthenticationMixin, MeetingMixin, ResourceMixin):
    ...
    def test_view_and_edit_access(self):
        ...
        # unbook the resource
        response = self.rest_update_meeting_remove_resource(self.token1, self.meeting1.pk, HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_trash_and_restore_access(self):
        ...
        response = self.rest_trash_meeting(self.token1, self.meeting1.pk, HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        # user1 should be able to restore the meeting as he created the meeting
        response = self.rest_restore_meeting(self.token1, self.meeting1.pk, HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
