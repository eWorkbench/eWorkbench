...
class TasksAssignedUsersTest(APITestCase, AuthenticationMixin, UserMixin, TaskMixin, ProjectsMixin):
    ...
    def test_assigned_users_permission(self):
        ...
        response = self.rest_create_task(self.token1, None, "Important Task", "Please do it ASAP",
                                         Task.TASK_STATE_NEW, Task.TASK_PRIORITY_HIGH, datetime.now(),
        ...
        response = self.rest_update_task(self.token1, task1['pk'], None, task1['title'] + "!!!11",
                                         task1['description'] + "!", task1['state'], task1['priority'],
                                         task1['start_date'], task1['due_date'], task1['assigned_users'][0]['pk'],
                                         HTTP_USER_AGENT, REMOTE_ADDR)

        self.assertEquals(response.status_code, status.HTTP_200_OK)

        # however, user2 should not be able to change this task
        response = self.rest_update_task(self.token2, task1['pk'], None, task1['title'] + "....",
                                         task1['description'] + "?!?!?", task1['state'], task1['priority'],
                                         task1['start_date'], task1['due_date'], task1['assigned_users'][0]['pk'],
                                         HTTP_USER_AGENT, REMOTE_ADDR)
        ...
        response = self.rest_get_task(
            self.token2, task1['pk'], HTTP_USER_AGENT, REMOTE_ADDR
        )

        self.assertIn(response.status_code, [status.HTTP_404_NOT_FOUND, status.HTTP_403_FORBIDDEN])

        # lets assign user2 to this task
        response = self.rest_update_task(self.token1, task1['pk'], None, task1['title'], task1['description'],
                                         task1['state'], task1['priority'], task1['start_date'], task1['due_date'],
                                         self.user2.pk, HTTP_USER_AGENT, REMOTE_ADDR)
        ...
        response = self.rest_update_task(self.token2, task1['pk'], None, task1['title'], "No way!", task1['state'],
                                         task1['priority'], task1['start_date'], task1['due_date'],
                                         task1['assigned_users'][0]['pk'], HTTP_USER_AGENT, REMOTE_ADDR)

        self.assertIn(response.status_code, [status.HTTP_404_NOT_FOUND, status.HTTP_403_FORBIDDEN])

        # user1 is still able to edit the task, as user1 created the task
        response = self.rest_update_task(self.token1, task1['pk'], None, task1['title'], "No way!", task1['state'],
                                         task1['priority'], task1['start_date'], task1['due_date'],
                                         task1['assigned_users'][0]['pk'], HTTP_USER_AGENT, REMOTE_ADDR)
    ...
    def test_multiple_assigned_users(self):
        ...
        response = self.rest_update_task_assigned_users(self.token2, task1['pk'], [self.user1.pk, self.user2.pk],
                                                        HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertIn(response.status_code, [status.HTTP_404_NOT_FOUND, status.HTTP_403_FORBIDDEN])

        # verify the number of assigned user for this task
        self.assertEquals(len(Task.objects.filter(pk=task1['pk']).first().assigned_users.all()), 1,
                          msg="Verify there is only one assigned user")
        # verify this user is user1
        self.assertEquals(Task.objects.filter(pk=task1['pk']).first().assigned_users.all().first().pk, self.user1.pk)

        # try to assign another user to this task
        response = self.rest_update_task_assigned_users(self.token1, task1['pk'], [self.user1.pk, self.user2.pk],
                                                        HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_200_OK)
        ...
        self.assertEquals(len(Task.objects.filter(pk=task1['pk']).first().assigned_users.all()), 2,
                          msg="Verify there are two assigned user")
        # try to remove user1 with user2 (should work)
        response = self.rest_update_task_assigned_users(self.token2, task1['pk'], [self.user2.pk],
                                                        HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_200_OK)
