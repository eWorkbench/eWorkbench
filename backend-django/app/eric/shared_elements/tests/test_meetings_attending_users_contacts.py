...
from eric.shared_elements.models import Contact, Meeting
...
class MeetingAttendingUsersContactsTest(
    APITestCase,
    AuthenticationMixin,
    ProjectsMixin,
    ContactMixin,
    ...
    def setUp(self):
        ...
        self.project = self.create_project(
            self.token1,
            "New Project",
            "Unittest User / Contact attends meetings Project",
            Project.INITIALIZED,
            HTTP_USER_AGENT,
            REMOTE_ADDR,
        )
        ...
        response = self.rest_create_contact(
            self.token1, self.project.pk, "", "First", "NumberOne", "first@eric.net", HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assert_response_status(response, status.HTTP_201_CREATED)
        decoded = json.loads(response.content.decode())
        ...
        # contact 2
        response = self.rest_create_contact(
            self.token1, self.project.pk, "", "Second", "NumberTwo", "second@eric.net", HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assert_response_status(response, status.HTTP_201_CREATED)
        decoded = json.loads(response.content.decode())
        ...
        response = self.rest_create_contact(
            self.token1, None, "", "Third", "NumberThree", "third@eric.net", HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assert_response_status(response, status.HTTP_201_CREATED)
        decoded = json.loads(response.content.decode())
    ...
    def test_add_and_delete_attending_user_with_two_users(self):
        ...
        response = self.update_attending_users(
            self.token2, self.meeting1.pk, user_pk_list, HTTP_USER_AGENT, REMOTE_ADDR
        )
    ...
    def test_create_attending_contact(self):
        ...
        response = self.update_attending_contacts(
            self.token1, self.meeting1.pk, contact_pk_list, HTTP_USER_AGENT, REMOTE_ADDR
        )
    ...
    def test_add_and_delete_attending_contact_with_two_users(self):
        ...
        response = self.update_attending_contacts(
            self.token2, self.meeting1.pk, contact_pk_list, HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assert_response_status(response, status.HTTP_200_OK)
        decoded_response = json.loads(response.content.decode())
        self.assertEqual(len(decoded_response["attending_contacts"]), 1, msg="Should be exactly one attending contact")
    ...
    def test_current_user_is_always_attending(self):
        ...
        self.assertEqual(meeting.attending_users.all().first().pk, self.user1.pk)
        self.assertEqual(decoded["attending_users_pk"][0], self.user1.pk)
    ...
    def test_attending_users_permission(self):
        ...
        self.assertEqual(len(decoded["attending_users_pk"]), 1)
        self.assertEqual(decoded["attending_users_pk"][0], self.user1.pk)
        ...
        response = self.update_attending_users(
            self.token1, self.meeting1.pk, [self.user1.pk, self.user2.pk], HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assert_response_status(response, status.HTTP_200_OK)
        ...
        self.assertEqual(
            Meeting.objects.filter(pk=decoded["pk"]).first().attending_users.all().count(),
            2,
    ...
    def test_meeting_with_invalid_attending_contact(self):
        ...
        response = self.update_attending_contacts(self.token2, self.meeting1.pk, [], HTTP_USER_AGENT, REMOTE_ADDR)
        self.assert_response_status(response, status.HTTP_200_OK)
    ...
    def test_create_meeting_with_attending_users_and_contacts(self):
        ...
        response = self.rest_create_meeting(
            ...
            [self.contact1.pk, self.contact2.pk],
        )
        decoded_meeting = json.loads(response.content.decode())
        self.assert_response_status(response, status.HTTP_201_CREATED)
        # verify a new meeting has been created in database
        self.assertEqual(Meeting.objects.all().count(), meeting_count + 1)
        meeting = Meeting.objects.filter(pk=decoded_meeting["pk"]).first()
        self.assertEqual(meeting.attending_users.count(), 2)
        self.assertEqual(meeting.attending_contacts.count(), 2)
    ...
    def test_meeting_update_attending_users_without_permission(self):
        ...
        response = self.update_attending_users(
            self.token1, self.meeting1.pk, [self.user1.pk], HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assert_response_status(response, status.HTTP_200_OK)

        # try to remove it again with user2 (token2) -> should not work, as user 2 is not allowed to view the meeting
        response = self.update_attending_users(self.token2, self.meeting1.pk, [], HTTP_USER_AGENT, REMOTE_ADDR)
        self.assert_response_status(response, status.HTTP_404_NOT_FOUND)
        ...
        self.assertEqual(self.meeting1.attending_users.all().count(), 0, msg="there should be zero attending users")
    ...
    def test_meeting_update_attending_contacts_without_permission(self):
        ...
        # try with user1 (token1) -> should work
        response = self.update_attending_contacts(
            self.token1, self.meeting1.pk, [self.contact1.pk], HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assert_response_status(response, status.HTTP_200_OK)
        self.assertEqual(
            self.meeting1.attending_contacts.all().count(), 1, msg="there should be zero attending contacts"
        )
        ...
        # also verify that attending users of this meeting are still set to two
        self.assertEqual(
            self.meeting1.attending_contacts.all().count(), 0, msg="there should be zero attending contacts"
        )
