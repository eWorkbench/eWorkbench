...
class TestGenericsContacts(APITestCase, EntityChangeRelatedProjectTestMixin, MeetingMixin, ContactMixin):
    ...
    def test_contact_privileges_for_meeting_with_attending_users(self):
        ...
        response = self.rest_get_privileges_for_user(
            self.token2, "contacts", contact.pk, self.user2.pk, HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        decoded_response = json.loads(response.content.decode())
        self.assertEqual(decoded_response["user_pk"], self.user2.pk)
        self.assertEqual(decoded_response["view_privilege"], "AL")
