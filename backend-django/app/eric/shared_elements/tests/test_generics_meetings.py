...
class TestGenericMeetings(APITestCase, EntityChangeRelatedProjectTestMixin, MeetingMixin):
    ...
    def test_meeting_privileges_for_attending_users(self):
        ...
        self.assertEquals(len(decoded_list), 0, msg="User 2 should see 0 meetings")

        # add user2 as an assigned user of the meeting
        response = self.rest_update_meeting_attending_users(self.token1, meeting.pk, [self.user2.pk],
    ...
    def test_create_meeting_with_wrong_time_validation_errors(self):
        ...
        :return:
        """
        response = self.rest_create_meeting(self.token1, None, "Some Meeting Title", "Some Meeting Description",
