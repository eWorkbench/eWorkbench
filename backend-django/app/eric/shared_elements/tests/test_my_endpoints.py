...
from eric.projects.tests.core import AuthenticationMixin, MeMixin, ProjectsMixin, UserMixin
...
class MyEndpointsTest(APITestCase, AuthenticationMixin, UserMixin, TaskMixin, MeetingMixin, ProjectsMixin, MeMixin):
    ...
    def setUp(self):
        ...
        self.add_meeting_without_project_permission = Permission.objects.filter(
            codename="add_meeting_without_project", content_type=Meeting.get_content_type()
        ).first()
    ...
    def test_api_me(self):
        ...
        self.assertEqual(decoded["username"], "student_1")
        ...
        self.assertEqual(decoded["username"], "student_2")
    ...
    def test_api_my_schedule(self):
        ...
        meeting1, response = self.create_meeting_orm(
            self.token1,
            None,
            "Meeting title",
            "meeting description",
        ...
        meeting2, response = self.create_meeting_orm(
            self.token1,
            None,
            "Another meeting title",
            "Another meeting description",
            datetime.now(),
            datetime.now(),
            HTTP_USER_AGENT,
            REMOTE_ADDR,
        ...
        response = self.client.get(
            f"/api/my/schedule/?show_meetings_for={self.user1.pk}",
        ...
        self.assertEqual(len(decoded), 4, msg="The my schedule endpoint should return 4 entities")
        ...
        response = self.client.get("/api/my/tasks/", HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR)
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        decoded = json.loads(response.content.decode())

        self.assertEqual(len(decoded), 2, msg="The my tasks endpoint should return 2 entities")
        self.assertEqual(decoded[0]["content_type_model"], "shared_elements.task")
        self.assertEqual(decoded[1]["content_type_model"], "shared_elements.task")

        # query the my meetings endpoint
        response = self.client.get("/api/my/meetings/", HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR)
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        decoded = json.loads(response.content.decode())

        self.assertEqual(len(decoded), 2, msg="The my meetings endpoint should return 2 entities")
        self.assertEqual(decoded[0]["content_type_model"], "shared_elements.meeting")
        self.assertEqual(decoded[1]["content_type_model"], "shared_elements.meeting")
