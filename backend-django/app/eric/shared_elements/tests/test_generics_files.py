...
class TestGenericsFiles(APITestCase, EntityChangeRelatedProjectTestMixin, FileMixin):
    ...
    def setUp(self):
        ...
        self.add_file_permission = Permission.objects.filter(
            codename="add_file", content_type=File.get_content_type()
        ).first()

        self.add_file_without_project_permission = Permission.objects.filter(
            codename="add_file_without_project", content_type=File.get_content_type()
        ).first()
    ...
    def test_download_file_with_permission(self):
        ...
        response = self.rest_download_file(self.token1, file.file_entries.all()[1].pk, HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.get("Content-Type"), "text/plain;")
        self.assertTrue("attachment; filename=" in response.get("Content-Disposition"))

        # try to to do the same with another user which does not have access to the project
        response = self.rest_download_file(self.token2, file.file_entries.all()[0].pk, HTTP_USER_AGENT, REMOTE_ADDR)
        ...
        self.rest_edit_user_project_assignment(
            self.token1, project, assignment["pk"], self.user2, self.pm_role, HTTP_USER_AGENT, REMOTE_ADDR
        )
    ...
    def test_create_file_with_and_without_permission(self):
        ...
        self.assertEqual(File.objects.all().count(), 0, msg="There should be zero files to begin with")

        # try creating a file without a project and without having the proper permission
        response = self.rest_create_file(
            self.token3, None, "Test Title", "Test Description", "somefile.txt", 1024, HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertIn(response.status_code, [status.HTTP_403_FORBIDDEN, status.HTTP_400_BAD_REQUEST])

        # there should still be zero files
        self.assertEqual(File.objects.all().count(), 0, msg="There should still be zero files")
        ...
        self.assertEqual(File.objects.all().count(), 2, msg="There should be two files in the database")
        ...
        self.assertEqual(File.objects.all().count(), 3, msg="There should be three files in the database")
    ...
    def test_duplicate_file(self):
        ...
        self.assertEqual(File.objects.all().count(), 2, msg="there should be two files")

        # try to duplicate file2 with user1 (should not work)
        response = self.rest_duplicate_file(
            self.token1, None, "Duplicate title", "some description", file2["pk"], HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

        self.assertEqual(File.objects.all().count(), 2, msg="there should still be two files")

        # try to duplicate file1 with user2 (should not work)
        response = self.rest_duplicate_file(
            self.token2, None, "Duplicate title", "some description", file1["pk"], HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        ...
        response = self.rest_generic_patch_privilege(
            self.token2, file2["pk"], self.user1.pk, {"view_privilege": ModelPrivilege.ALLOW}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        self.assertEqual(File.objects.all().count(), 3, msg="there should now be three files")
    ...
    def test_create_file_with_unicode_characters(self):
        ...
        # try creating a file without a project and without having the proper permission
        response = self.rest_create_file(
            self.token1, None, "Test Title", "Test Description", "some😀😎file.txt", 1024, HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_on_delete_file_also_delete_the_physical_files(self):
        ...
        # create a file with 1024 bytes
        response = self.rest_create_file(
            self.token1, None, "Test Title", "Test Description", "somefile.txt", 1024, HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
