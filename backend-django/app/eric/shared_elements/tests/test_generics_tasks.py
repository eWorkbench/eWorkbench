...
class TestGenericsTasks(APITestCase, EntityChangeRelatedProjectTestMixin, TaskMixin, ElementLabelMixin):
    ...
    def test_task_privileges_for_assigned_users(self):
        ...
        self.assertEqual(len(decoded_list), 0, msg="User 2 should see 0 tasks")

        # add user2 as an assigned user of the task
        response = self.rest_update_task_assigned_users(
            self.token1, task.pk, [self.user2.pk], HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        self.assertEqual(decoded_privileges[1]["restore_privilege"], ModelPrivilege.NEUTRAL)
        ...
        response = self.rest_update_privilege(
            self.token1, "tasks", task.pk, self.user2.pk, decoded_privileges[1], HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        self.assertEqual(decoded_privileges[1]["full_access_privilege"], ModelPrivilege.NEUTRAL)
        self.assertEqual(decoded_privileges[1]["view_privilege"], ModelPrivilege.DENY)
        self.assertEqual(decoded_privileges[1]["edit_privilege"], ModelPrivilege.DENY)
        self.assertEqual(decoded_privileges[1]["delete_privilege"], ModelPrivilege.DENY)
        self.assertEqual(decoded_privileges[1]["restore_privilege"], ModelPrivilege.DENY)
    ...
    def test_create_update_task_checklist(self):
        ...
        self.assertEqual(TaskCheckList.objects.all().count(), 0)
        ...
        self.assertEqual(len(decoded_response["checklist_items"]), len(task_data["checklist_items"]))

        # verify checklist items have been created
        self.assertEqual(TaskCheckList.objects.all().count(), len(task_data["checklist_items"]))
        # verify that all those checklist items have the task instance set
        self.assertEqual(
            TaskCheckList.objects.filter(task=decoded_response["pk"]).count(), len(task_data["checklist_items"])
        )
        # verify that only one of them is checked
        self.assertEqual(TaskCheckList.objects.filter(task=decoded_response["pk"], checked=True).count(), 1)
        ...
        self.assertEqual(TaskCheckList.objects.filter(task=decoded_response["pk"], checked=True).count(), 2)
    ...
    def test_projects_task_state_summary(self):
        ...
        response = self.rest_create_project(
            self.token1, "My Master Project", "Some description", Project.INITIALIZED, HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        ...
        response = self.rest_create_project(
            self.token1, "My Sub Project", "Some description", Project.INITIALIZED, HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        ...
        self.assertEqual(project2.parent_project.pk, project1.pk)
        ...
        self.assertEqual(decoded_response["tasks_status"][Task.TASK_STATE_NEW], 1)
        self.assertEqual(decoded_response["tasks_status"][Task.TASK_STATE_PROGRESS], 0)
        self.assertEqual(decoded_response["tasks_status"][Task.TASK_STATE_DONE], 0)
        ...
        self.assertEqual(decoded_response["tasks_status"][Task.TASK_STATE_NEW], 0)
        self.assertEqual(decoded_response["tasks_status"][Task.TASK_STATE_PROGRESS], 0)
        self.assertEqual(decoded_response["tasks_status"][Task.TASK_STATE_DONE], 0)
        ...
        self.assertEqual(decoded_response["tasks_status"][Task.TASK_STATE_NEW], 0)
        self.assertEqual(decoded_response["tasks_status"][Task.TASK_STATE_PROGRESS], 0)
        self.assertEqual(decoded_response["tasks_status"][Task.TASK_STATE_DONE], 1)
        ...
        response = self.rest_update_task_project(
            self.token1, task2.pk, [project1.pk, project2.pk], HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_task_element_labels(self):
        ...
        # create a new label
        response = self.rest_create_label(
            self.token1, "Important Label", "rgba(50,50,50,1)", HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        ...
        self.assertEqual(task.labels.all().count(), 2)
