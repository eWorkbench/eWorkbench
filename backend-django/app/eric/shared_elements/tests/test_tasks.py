...
class TasksTest(APITestCase, AuthenticationMixin, UserMixin, TaskMixin, ProjectsMixin):
    ...
    def test_user_can_only_see_tasks_created_by_own_user(self):
        ...
        response = self.rest_create_task(self.token2, None,
                                         "Test Task", "Test Description",
                                         Task.TASK_STATE_NEW, Task.TASK_PRIORITY_HIGH, datetime.now(),
                                         datetime.now() + timedelta(days=30),
                                         self.user2.pk,
    ...
    def test_create_and_edit_of_own_task(self):
        ...
        self.assertEquals(Task.objects.all().count(), 1, msg="There should be one task in the database")

        # try edit this task with user1
        response = self.rest_update_task(self.token1, task.pk, None, "Test Task", "Test Description",
                                         Task.TASK_STATE_NEW, Task.TASK_PRIORITY_HIGH, datetime.now(),
                                         datetime.now() + timedelta(days=30), self.user1.pk, HTTP_USER_AGENT,
                                         REMOTE_ADDR)
        ...
        response = self.rest_update_task(self.token2, decoded['pk'], None, "Test Task", "Test Description",
                                         Task.TASK_STATE_NEW, Task.TASK_PRIORITY_HIGH, datetime.now(),
                                         datetime.now() + timedelta(days=30), self.user1.pk, HTTP_USER_AGENT,
                                         REMOTE_ADDR)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
