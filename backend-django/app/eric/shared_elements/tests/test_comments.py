...
class CommentsTest(APITestCase, AuthenticationMixin, UserMixin, CommentMixin, ProjectsMixin):
    ...
    def setUp(self):
        ...
        self.add_comment_permission = Permission.objects.filter(
            codename="add_comment", content_type=Comment.get_content_type()
        ).first()

        self.add_comment_without_project_permission = Permission.objects.filter(
            codename="add_comment_without_project", content_type=Comment.get_content_type()
        ).first()
    ...
    def test_create_comment_with_and_without_permission(self):
        ...
        self.assertEqual(Comment.objects.all().count(), 0, msg="There should be zero Comments to begin with")

        # try creating a comment without a project and without having the proper permission
        response = self.rest_create_comment(self.token3, None, "Test Description", HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertIn(response.status_code, [status.HTTP_403_FORBIDDEN, status.HTTP_400_BAD_REQUEST])

        # there should still be zero Comments
        self.assertEqual(Comment.objects.all().count(), 0, msg="There should still be zero Comments")

        # however, creating a comment for a project1 should work, as user1 has created project1 (and therefore should have
        # the needed permissions)
        response = self.rest_create_comment(
            self.token3, self.project1.pk, "Test Description", HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        ...
        self.assertEqual(Comment.objects.all().count(), 2, msg="There should be two Comments in the database")
        ...
        self.assertEqual(Comment.objects.all().count(), 3, msg="There should be three Comments in the database")
    ...
    def test_get_comments_with_filter(self):
        ...
        response = self.rest_create_comment(self.token1, None, "Test Description", HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        ...
        self.assertEqual(decoded["pk"], str(comment.pk))

        ########
        # create a comment for project1
        ########
        response = self.rest_create_comment(
            self.token1, self.project1.pk, "Test Description", HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_user_can_only_see_Comments_created_by_own_user(self):
        ...
        response = self.rest_create_comment(self.token2, None, "Test Description", HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_create_and_get_comments(self):
        ...
        # create a comment
        comment, response = self.create_comment_orm(
            self.token1, project.pk, "Test Description", HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        ...
        response = self.rest_create_comment(
            self.token1, project.pk, "Another Test Description", HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        ...
        response = self.rest_update_comment(
            self.token1, comment.pk, project.pk, "Test Comment Description", HTTP_USER_AGENT, REMOTE_ADDR
        )
        decoded = json.loads(response.content.decode())
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_create_and_edit_of_own_comment(self):
        ...
        comment, response = self.create_comment_orm(self.token1, None, "Test Description", HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

        # there should now be one comment
        self.assertEqual(Comment.objects.all().count(), 1, msg="There should be one comment in the database")

        # try edit this comment with user1
        response = self.rest_update_comment(
            self.token1, comment.pk, None, "Test Description", HTTP_USER_AGENT, REMOTE_ADDR
        )
        ...
        response = self.rest_update_comment(
            self.token2, decoded["pk"], None, "Test Description", HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
