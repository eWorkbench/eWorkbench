...
from eric.shared_elements.models import CalendarAccess, Comment, Contact, ElementLabel, File, Meeting, Note, Task
...
class FileFilter(WorkbenchElementFilter):
    ...
    container = django_filters.ModelChoiceFilter(
        field_name="directory__drive__envelope__container", queryset=DSSContainer.objects.all()
    )
...
class ElementLabelFilter(BaseFilter):
    class Meta:
        model = ElementLabel
        fields = {"name": BaseFilter.STRING_COMPERATORS}
