...
from eric.shared_elements.rest.serializers import MinimalisticMeetingSerializer, MinimalisticTaskSerializer
...
class MyScheduleViewSet(BaseGenericViewSet):
    ...
    def get_meetings_queryset(self, show_meetings_for):
        ...
        qs_extended = (
            ...
            .exclude(attending_users=current_user)
            .filter(
    ...
    def list(self, request, *args, **kwargs):
        ...
        serialized_tasks = MinimalisticTaskSerializer(tasks, many=True, context={"request": self.request}).data

        serialized_meetings = MinimalisticMeetingSerializer(meetings, many=True, context={"request": self.request}).data
    ...
    def get_ical_export_link(self, request, *args, **kwargs):
        ...
        return Response({"url": short_url.get_short_url()})
    ...
    def ical_export(self, *args, **kwargs):
        ...
        for task in tasks:
            ...
            event.add("description").value = convert_html_to_text(task.description)  # description (without html)

            # creator of the task
            event.add("organizer").value = "CN:{organizer_name}:MAILTO:{organizer_mail}".format(
