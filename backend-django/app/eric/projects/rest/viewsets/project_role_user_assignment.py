...
from eric.projects.rest.serializers import ChangeSetSerializer, ProjectRoleUserAssignmentSerializerExtended
...
class ProjectRoleUserAssignmentViewSet(BaseAuthenticatedNestedProjectModelViewSet):
    ...
    def get_queryset(self):
        ...
        if "project_pk" in self.kwargs:
            return (
                ProjectRoleUserAssignment.objects.viewable()
                .filter(project=self.kwargs["project_pk"])
                .select_related("role", "user", "user__userprofile", "project")
            )
        else:
            return ProjectRoleUserAssignment.objects.viewable().select_related(
    ...
    def get_assigned_users_up(self, request, *args, **kwargs):
        ...
        qs = (
            ProjectRoleUserAssignment.objects.viewable()
            .filter(project__in=parent_project_pks)
            .select_related("role", "user", "user__userprofile", "project")
        )
