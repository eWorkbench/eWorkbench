...
from eric.projects.models import Project, ProjectRoleUserAssignment, Role, UserStorageLimit
from eric.projects.rest.serializers.changeset import ChangeRecordSerializer, ChangeSetSerializer
...
class ProjectRoleUserAssignmentSerializerExtended(BaseModelWithCreatedBySerializer):
    ...
    user_pk = serializers.PrimaryKeyRelatedField(queryset=User.objects.all(), source="user", many=False, required=True)
    ...
    role_pk = serializers.PrimaryKeyRelatedField(queryset=Role.objects.all(), source="role", many=False, required=True)
...
class ProjectSerializerExtended(BaseModelWithCreatedByAndSoftDeleteSerializer, EntityMetadataSerializerMixin):
    ...
    # url for ACL-list
    acls = HyperlinkedToListField(view_name="projectroleuserassignment-list", lookup_field_name="project_pk")

    # url for breadcrumb-list
    breadcrumbs = HyperlinkedToListField(view_name="projectbreadcrumb-list", lookup_field_name="project_pk")

    # url for contact-list
    contacts = HyperlinkedQueryToListField(view_name="contact-list", lookup_field_name="project")

    # url for dmp-list
    dmps = HyperlinkedQueryToListField(view_name="dmp-list", lookup_field_name="project")

    # url for file-list
    files = HyperlinkedQueryToListField(view_name="file-list", lookup_field_name="project")

    # url for meeting-list
    meetings = HyperlinkedQueryToListField(view_name="meeting-list", lookup_field_name="project")

    # url for meeting-list
    resources = HyperlinkedQueryToListField(view_name="resource-list", lookup_field_name="project")

    # url for note-list
    notes = HyperlinkedQueryToListField(view_name="note-list", lookup_field_name="project")

    # url for task-list
    tasks = HyperlinkedQueryToListField(view_name="task-list", lookup_field_name="project")

    # url for changeset-list
    history = HyperlinkedToListField(view_name="projects_changeset-list", lookup_field_name="project_pk")
    ...
    def get_tasks_status(self, instance):
        ...
        tasks = (
            Task.objects.for_project(instance.pk)
...
class MyUserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = (
            ...
            "is_active",
            "permissions",
            "used_storage_megabyte",
            "available_storage_megabyte",
        )
        ...
        with transaction.atomic():
            ...
            if profile_data is not None:
                ...
                instance.userprofile = userprofile_serializer.update(
                    instance.userprofile, userprofile_serializer.validated_data
                )
