...
class DuplicateProjectTest(APITestCase, AuthenticationMixin, ProjectsMixin, TaskMixin):
    ...
    def setUp(self):
        ...
        self.user1 = User.objects.create_user(username="user_1", email="user_1@email.com", password="top_secret")
        self.user1.groups.add(self.user_group)

        self.user2 = User.objects.create_user(username="user_2", email="user_2@email.com", password="top_secret")
        self.user2.groups.add(self.user_group)

        self.user3 = User.objects.create_user(username="user_3", email="user_3@email.com", password="top_secret")
        ...
        project = self.create_project(
            self.token1, "Project", "Project Description", Project.INITIALIZED, HTTP_USER_AGENT, REMOTE_ADDR
        )
        ...
        sub_project_1 = self.create_project(
            self.token1, "Sub Project 1", "Project Description", Project.INITIALIZED, HTTP_USER_AGENT, REMOTE_ADDR
        )
        ...
        self.rest_create_task(
            self.token1,
            [self.sub_project_1_pk],
            "Test Title Sub Task 1",
            "Test Description",
            "NEW",
            "2",
            datetime.now(),
        ...
        sub_sub_project_1 = self.create_project(
            self.token1, "Sub Sub Project 1", "Project Description", Project.INITIALIZED, HTTP_USER_AGENT, REMOTE_ADDR
        )
        ...
        self.rest_create_task(
            self.token1,
            [self.sub_sub_project_1_pk],
            "Test Title Sub Sub Task 1",
            "Test Description",
            "NEW",
            "2",
            datetime.now(),
        ...
        sub_project_2 = self.create_project(
            self.token1, "Sub Project 2", "Project Description", Project.INITIALIZED, HTTP_USER_AGENT, REMOTE_ADDR
        )
        ...
        self.rest_create_task(
            self.token1,
            [self.sub_project_2_pk],
            "Test Title Sub Task 2",
            "Test Description",
            "NEW",
            "2",
            datetime.now(),
        ...
        self.rest_assign_user_to_project(
            self.token1, project, self.user2, self.observer_role, HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.rest_assign_user_to_project(
            self.token1, project, self.user3, self.observer_role, HTTP_USER_AGENT, REMOTE_ADDR
        )
    ...
    def test_duplicate_parent_project(self):
        ...
        self.assertEqual(
            name,
            "Copy of %s" % (original_project["name"]),
        ...
        self.assertEqual(
            len(duplicated_project_tasks),
            len(original_project_tasks),
            msg="The duplicate project has to have the same count of tasks than the original " "project",
        )

        self.assertNotEqual(
            duplicated_project_tasks[0].pk,
            original_project_tasks[0].pk,
        ...
        self.assertEqual(
            duplicated_project_tasks[0].title,
            "Copy of %s" % (original_project_tasks[0].title),
        ...
        self.assertEqual(
            len(duplicated_sub_projects),
            len(original_sub_projects),
            msg="The duplicate project has to have the same count of sub projects than the original " "project",
        )

        self.assertNotEqual(
            duplicated_sub_projects[0].pk,
            original_sub_projects[0].pk,
            msg="The sub project PK of the duplicated project is not the same as the sub project pk of "
            "the original project",
        )
        self.assertEqual(
            duplicated_sub_projects[0].name,
            original_sub_projects[0].name,
            msg="The sub project name of the duplicated project is the same as the sub project name of "
            "the original project",
        )

        self.assertNotEqual(
            duplicated_sub_projects[1].pk,
            original_sub_projects[1].pk,
            msg="The sub project PK of the duplicated project is not the same as the sub project pk of "
            "the original project",
        )
        self.assertEqual(
            duplicated_sub_projects[1].name,
            original_sub_projects[1].name,
            msg="The sub project name of the duplicated project is the same as the sub project name of "
            "the original project",
        )
        ...
        # check this sub project task
        self.assertEqual(
            len(duplicated_sub_project_tasks),
            len(original_sub_project_tasks),
            msg="The duplicate project has to have the same count of tasks than the original " "project",
        )

        self.assertNotEqual(
            duplicated_sub_project_tasks[0].pk,
            original_sub_project_tasks[0].pk,
            msg="The sub project task PK of the duplicated sub project task is not the same "
            "as the sub project task pk of the original sub project task",
        )

        self.assertEqual(
            duplicated_sub_project_tasks[0].title,
            "Copy of %s" % (original_sub_project_tasks[0].title),
            msg="The sub project task title of the duplicated sub project is the same as the sub"
            "project task title of the original sub project with a 'Copy of' in front of it",
        )
        ...
        self.assertEqual(
            len(duplicated_sub_sub_project),
            len(original_sub_sub_project),
            msg="The duplicate project has to have the same count of sub projects than the original " "project",
        )

        self.assertNotEqual(
            duplicated_sub_sub_project[0].pk,
            original_sub_sub_project[0].pk,
        ...
        self.assertEqual(
            duplicated_sub_sub_project[0].name,
            original_sub_sub_project[0].name,
        ...
        # check this sub project task
        self.assertEqual(
            len(duplicated_sub_sub_project_tasks),
            len(original_sub_sub_project_tasks),
            msg="The duplicate project has to have the same count of tasks than the original " "project",
        )

        self.assertNotEqual(
            duplicated_sub_sub_project_tasks[0].pk,
            original_sub_sub_project_tasks[0].pk,
            msg="The sub project task PK of the duplicated sub project task is not the same "
            "as the sub project task pk of the original sub project task",
        )

        self.assertEqual(
            duplicated_sub_sub_project_tasks[0].title,
            "Copy of %s" % (original_sub_sub_project_tasks[0].title),
            msg="The sub project task title of the duplicated sub project is the same as the sub"
            "project task title of the original sub project with a 'Copy of' in front of it",
        )
    ...
    def test_duplicate_sub_project(self):
        ...
        self.assertEqual(
            decoded["parent_project"], None, msg="The duplicated sub project has now no parent project anymore"
        )
        ...
        # check this sub project
        self.assertEqual(
            len(duplicated_sub_project),
            len(original_sub_project),
            msg="The duplicate project has to have the same count of sub projects than the original " "project",
        )

        self.assertNotEqual(
            duplicated_sub_project[0].pk,
            original_sub_project[0].pk,
            msg="The sub project PK of the duplicated project is not the same as the sub"
            "project pk of the original project",
        )
        self.assertEqual(
            duplicated_sub_project[0].name,
            original_sub_project[0].name,
    ...
    def test_duplicate_project_without_sub_projects(self):
        ...
        self.assertEqual(len(duplicated_sub_project), 0, msg="The count of sub projects has to be 0.")
    ...
    def test_duplicate_project_with_other_users(self):
        ...
        self.assertEqual(
            response.status_code, status.HTTP_403_FORBIDDEN, msg="User is not allowed to duplicate (create) a project"
        )
    ...
    def test_duplicate_trashed_project(self):
        ...
        decoded = json.loads(response.content.decode())
        self.assertEqual(decoded["deleted"], True, msg="The project should now be trashed")
    ...
    def test_duplicate_project_with_trashed_sub_project(self):
        ...
        decoded = json.loads(response.content.decode())
        self.assertEqual(decoded["deleted"], True, msg="The sub project should now be trashed")
        ...
        for duplicated_sub_project in duplicated_sub_projects:
            self.assertEqual(
                duplicated_sub_project.deleted,
                False,
