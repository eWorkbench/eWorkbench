...
from eric.shared_elements.tests.core import ContactMixin, NoteMixin, TaskMixin
...
class HistoryTest(APITestCase, AuthenticationMixin, ProjectsMixin, TaskMixin, ContactMixin):
    ...
    def test_project_changeset(self):
        ...
        response = self.client.get(
            f"/api/projects/{self.project1.pk}/history/",
        ...
        self.assertEqual(decoded[0]["changeset_type"], "I")
        # by user1
        self.assertEqual(decoded[0]["user"]["username"], "student_1")
        # for project with pk
        self.assertEqual(decoded[0]["object_uuid"], str(self.project1.pk))

        response = self.client.get(
            f"/api/projects/{self.project2.pk}/history/",
            HTTP_USER_AGENT=HTTP_USER_AGENT,
            REMOTE_ADDR=REMOTE_ADDR,
        )
        ...
        self.assertEqual(decoded[0]["changeset_type"], "I")
        # by user1
        self.assertEqual(decoded[0]["user"]["username"], "student_2")
        self.assertEqual(decoded[0]["object_type"]["model"], "projectroleuserassignment")

        # this should be an insert of the project
        self.assertEqual(decoded[1]["changeset_type"], "I")
        # by user1
        self.assertEqual(decoded[1]["user"]["username"], "student_2")
        self.assertEqual(decoded[1]["object_type"]["model"], "project")
        # for project with pk
        self.assertEqual(decoded[1]["object_uuid"], str(self.project2.pk))
    ...
    def test_history_permissions(self):
        ...
        self.assertEqual(history["count"], 1, msg="There should be exactly one history entry")

        history = history["results"]

        # should have 1 entry (create)
        self.assertEqual(len(history), 1, msg="There should be exactly one history entry")
        self.assertEqual(history[0]["user"]["pk"], self.user1.pk, msg="First history object was created by user1")
        self.assertEqual(history[0]["object_uuid"], task["pk"], msg="First history object is related to the given task")
