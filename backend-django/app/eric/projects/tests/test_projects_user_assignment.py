...
class ProjectsUserAssignmentTest(APITestCase, AuthenticationMixin, ProjectsMixin):
    ...
    def test_crud_project_user_assignments(self):
        ...
        response = self.rest_delete_user_from_project(
            self.token1, project.pk, decoded[0]["pk"], HTTP_USER_AGENT, REMOTE_ADDR
        )
        ...
        response = self.rest_edit_user_project_assignment(
            self.token1, project, decoded[0]["pk"], self.user1, self.student_role, HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

        # Try to remove user2 (student), which should work
        response = self.rest_delete_user_from_project(
            self.token1, project.pk, decoded[1]["pk"], HTTP_USER_AGENT, REMOTE_ADDR
        )
    ...
    def test_edit_project_user_assignments_twice(self):
        ...
        response = self.rest_assign_user_to_project(
            self.token1, project, self.user2, self.pm_role, HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_crud_project_user_assignments_change_project_manager(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            len(json.loads(response.content.decode())), 2, msg="There should be two users in the assignment"
        )

        # now user2 tries to increase his own roles to project manager (should not work)
        response = self.rest_edit_user_project_assignment(
            self.token2, project, decoded[1]["pk"], self.user2, self.pm_role, HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertIn(response.status_code, [status.HTTP_403_FORBIDDEN, status.HTTP_404_NOT_FOUND])

        # user2 tries to remove user1 from PM (should not work)
        response = self.rest_edit_user_project_assignment(
            self.token2, project, decoded[0]["pk"], self.user1, self.observer_role, HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertIn(
            response.status_code, [status.HTTP_400_BAD_REQUEST, status.HTTP_403_FORBIDDEN, status.HTTP_404_NOT_FOUND]
        )

        # Give user2 the project manager permission (should work)
        response = self.rest_edit_user_project_assignment(
            self.token1, project, decoded[1]["pk"], self.user2, self.pm_role, HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        # Now user2 can remove user1 (the first project manager), which should work
        response = self.rest_delete_user_from_project(
            self.token2, project.pk, decoded[0]["pk"], HTTP_USER_AGENT, REMOTE_ADDR
        )
        # this should work
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            len(json.loads(response.content.decode())), 1, msg="There should only be one user in the assignment"
        )
