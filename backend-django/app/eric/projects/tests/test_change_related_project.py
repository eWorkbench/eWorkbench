...
from eric.projects.models import Permission, Project, Role, RolePermissionAssignment
...
class ChangeRelatedProjectTest(APITestCase, AuthenticationMixin, ProjectsMixin, ContactMixin, TestLockMixin):
    ...
    def setUp(self):
        ...
        self.project1 = self.create_project(
            self.token1,
            "New Project 1",
            "Unittest update related project1",
            Project.INITIALIZED,
            HTTP_USER_AGENT,
            REMOTE_ADDR,
        )
        self.project2 = self.create_project(
            self.token1,
            "New Project 2",
            "Unittest update related project2",
            Project.INITIALIZED,
            HTTP_USER_AGENT,
            REMOTE_ADDR,
        )
        ...
        self.project3 = self.create_project(
            self.token2,
            "New Project 3",
            "Unittest update related project3",
            Project.INITIALIZED,
            HTTP_USER_AGENT,
            REMOTE_ADDR,
        )
        ...
        self.rest_assign_user_to_project(
            self.token1, self.project1, self.user2, self.test_role, HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.rest_assign_user_to_project(
            self.token1, self.project2, self.user2, self.test_role, HTTP_USER_AGENT, REMOTE_ADDR
        )
    ...
    def test_change_related_project_not_allowed(self):
        ...
        response = self.rest_update_contact(
            self.token1,
            self.contact1.pk,
            self.project2.pk,
            "",
    ...
    def test_change_related_project(self):
        ...
        change_related_project_permission = Permission.objects.filter(
            codename="change_project_contact", content_type=Contact.get_content_type()
        ).first()
        RolePermissionAssignment.objects.create(role=role, permission=change_related_project_permission)
