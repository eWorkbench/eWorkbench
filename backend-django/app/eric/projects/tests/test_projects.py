...
class ProjectsTest(APITestCase, AuthenticationMixin, ProjectsMixin, CommonTestMixin):
    ...
    def setUp(self):
        ...
        self.user3 = User.objects.create_user(
            username="student_3", email="student_3@email.com", password="very_secret")
        self.user3.groups.add(self.user_group)

        self.user4 = User.objects.create_user(
            username="professor", email="pro@fessor.com", password="12345678")
        ...
        self.assertEquals(Project.objects.all().count(), 0, msg="There should be zero projects in the database")
        ...
        self.assertEquals(Project.objects.all().count(), 4, msg="There should be four projects in the database")
    ...
    def test_login_auto_create_project(self):
        ...
        self.assertEquals(len(projects1), 1,
        ...
        self.assertEquals(len(projects2), 1,
        ...
        self.assertEquals(len(projects3), 1,
        ...
        self.assertEquals(len(projects4), 1,
        ...
        self.assertEquals(len(projects5), 0,
        ...
        self.assertNotEquals(projects1[0]['pk'], projects2[0]['pk'])
        self.assertNotEquals(projects1[0]['pk'], projects3[0]['pk'])
        self.assertNotEquals(projects1[0]['pk'], projects4[0]['pk'])
        self.assertNotEquals(projects2[0]['pk'], projects3[0]['pk'])
        self.assertNotEquals(projects2[0]['pk'], projects4[0]['pk'])
        self.assertNotEquals(projects3[0]['pk'], projects4[0]['pk'])
    ...
    def test_create_projects_with_different_users(self):
        ...
        self.assertEquals(Project.objects.all().count(), existing_project_count + 1,
        ...
        self.assertEquals(Project.objects.all().count(), 6)
        self.validate_number_of_projects_returned_from_rest(self.token1, 1 + 1, HTTP_USER_AGENT, REMOTE_ADDR)
        ...
        self.assertEquals(Project.objects.all().count(), 7)
        ...
        self.assertEquals(Project.objects.all().count(), 8)
        self.validate_number_of_projects_returned_from_rest(self.token1, 2, HTTP_USER_AGENT, REMOTE_ADDR)
        self.validate_number_of_projects_returned_from_rest(self.token2, 2, HTTP_USER_AGENT, REMOTE_ADDR)
        self.validate_number_of_projects_returned_from_rest(self.token3, 2, HTTP_USER_AGENT, REMOTE_ADDR)
    ...
    def test_delete_project(self):
        ...
        response = self.client.patch('/api/projects/{}/soft_delete/'.format(pro1.pk),
                                     HTTP_USER_AGENT=HTTP_USER_AGENT,
                                     REMOTE_ADDR=REMOTE_ADDR)
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        self.assertEqual(len(Project.objects.not_deleted().filter(pk=pro1.pk)), 0,
        ...
        response = self.rest_delete_project(self.superuser_token, pro1.pk,
                                            HTTP_USER_AGENT=HTTP_USER_AGENT,
                                            REMOTE_ADDR=REMOTE_ADDR)
