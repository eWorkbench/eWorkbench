...
from eric.projects.tests.core import AuthenticationMixin, ModelPrivilegeMixin, ProjectsMixin, UserMixin
...
class ModelPrivilegeTestCase(
    APITestCase, AuthenticationMixin, UserMixin, FileMixin, ProjectsMixin, ModelPrivilegeMixin
):
    """
    ...
    def test_create_new_object(self):
        ...
        """
        self.assertEqual(
            ModelPrivilege.objects.all().count(),
            3,
        ...
        response = self.rest_create_file(
            self.token1, None, "Some Title", "Some Description", "file.txt", 1024, HTTP_USER_AGENT, REMOTE_ADDR
        )
        ...
        self.assertEqual(model_privilege.user, self.user1)
        self.assertEqual(model_privilege.content_type, File.get_content_type())
        self.assertEqual(str(model_privilege.object_id), decoded_file_object["pk"])
        self.assertEqual(model_privilege.full_access_privilege, ModelPrivilege.ALLOW)

        # call rest api to get privileges for this file
        response = self.rest_get_privileges(
            self.token1, "files", decoded_file_object["pk"], HTTP_USER_AGENT, REMOTE_ADDR
        )
        ...
        self.assertEqual(len(decoded_privileges), 1, msg="There should be exactly one privilege")
        self.assertEqual(decoded_privileges[0]["object_id"], decoded_file_object["pk"])
        ...
        self.assertEqual(decoded_privileges[0]["view_privilege"], ModelPrivilege.NEUTRAL)
        self.assertEqual(decoded_privileges[0]["edit_privilege"], ModelPrivilege.NEUTRAL)
        self.assertEqual(decoded_privileges[0]["delete_privilege"], ModelPrivilege.NEUTRAL)
        self.assertEqual(decoded_privileges[0]["restore_privilege"], ModelPrivilege.NEUTRAL)
        self.assertEqual(decoded_privileges[0]["trash_privilege"], ModelPrivilege.NEUTRAL)
    ...
    def test_add_user_multiple_times_to_privileges(self):
        ...
        self.assertEqual(len(decoded_privileges), 2, msg="There should be two privileges")
    ...
    def test_delete_entity_delete_cascade_model_privileges(self):
        ...
        self.assertEqual(File.objects.all().count(), 0, msg="There should be zero files")
        self.assertEqual(
            ModelPrivilege.objects.all().count(),
            3,
            msg="There should only be three model privileges " "coming from calendar access privileges",
        )
