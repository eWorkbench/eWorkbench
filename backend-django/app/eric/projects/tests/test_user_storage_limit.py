...
from eric.projects.tests.core import AuthenticationMixin, MeMixin, ProjectsMixin
...
class UserStorageLimitTest(APITestCase, AuthenticationMixin, ProjectsMixin, FileMixin, MeMixin):
    ...
    def setUp(self):
        ...
        self.user2 = User.objects.create_user(username="student_2", email="student_2@email.com", password="top_secret")
        ...
        self.project = self.create_project(
            self.token1, "New Project", "User storage limit test", Project.INITIALIZED, HTTP_USER_AGENT, REMOTE_ADDR
        )
    ...
    def test_user_storage_limit_fields(self):
        ...
        self.assertEqual(
            decoded["available_storage_megabyte"], 100, msg="The limit of user storage limit should be 100 MB"
        )

        # check if the used storage is correct
        self.assertEqual(
            decoded["used_storage_megabyte"], 0, msg="The used storage is 0 because no files where uploaded yet"
        )
    ...
    def test_calculate_used_storage(self):
        ...
        response = self.rest_get_me(self.token1)
        decoded = json.loads(response.content.decode())
        self.assertEqual(decoded["used_storage_megabyte"] * MB_factor, 50263000, msg="The used storage is now 50.263")

        # create a file with 115000 bytes (115 KB)
        file2 = self.rest_create_file(
            self.token1,
            self.project.pk,
            "Test Title",
            "Test Description",
        ...
        decoded = json.loads(response.content.decode())
        self.assertEqual(
            decoded["used_storage_megabyte"] * MB_factor, 115000 + 50263000, msg="The used storage is now 50.378"
        )
        ...
        decoded = json.loads(response.content.decode())
        self.assertEqual(
            decoded["used_storage_megabyte"] * MB_factor,
            115000 + 50263000 + 8900000,
        ...
        file2 = self.rest_create_file(
            self.token2, None, "Test Title", "Test Description", "somefile.txt", 115000, HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(file2.status_code, status.HTTP_201_CREATED)
        ...
        decoded = json.loads(response.content.decode())
        self.assertEqual(decoded["used_storage_megabyte"] * MB_factor, 115000, msg="The used storage is now 0.115")
    ...
    def test_reach_user_storage_limit(self):
        ...
        decoded = json.loads(response.content.decode())
        self.assertEqual(decoded["available_storage"], 100.0, msg="The available storage should be 100.0")

        # check if the used storage is correct
        response = self.rest_get_me(self.token1)
        decoded = json.loads(response.content.decode())
        self.assertEqual(decoded["used_storage_megabyte"], 0, msg="The used storage is 0")

        # create a file with 115000 bytes (115 KB)
        file = self.rest_create_file(
            self.token1,
            self.project.pk,
            "Test Title",
            "Test Description",
        ...
        decoded = json.loads(response.content.decode())
        self.assertEqual(
            decoded["available_storage"], 100 - 115000 / 1024 / 1024, msg="The available storage should be 99.885000"
        )

        # check if the used storage is correct
        response = self.rest_get_me(self.token1)
        decoded = json.loads(response.content.decode())
        self.assertEqual(decoded["used_storage_megabyte"], 115000 / 1024 / 1024, msg="The used storage is 0.115")
