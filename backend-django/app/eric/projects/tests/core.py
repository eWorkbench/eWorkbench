...
from eric.core.tests import HTTP_INFO, HTTP_USER_AGENT, REMOTE_ADDR, custom_json_handler
from eric.projects.models import Project, ProjectRoleUserAssignment, Resource, Role, RolePermissionAssignment
...
class ModelPrivilegeMixin:
    ...
    def rest_get_privileges_for_user(self, auth_token, model, pk, user_pk, HTTP_USER_AGENT, REMOTE_ADDR):
        ...
        return self.client.get(
            f"/api/{model}/{pk}/privileges/{user_pk}/",
            HTTP_USER_AGENT=HTTP_USER_AGENT,
            REMOTE_ADDR=REMOTE_ADDR,
        )
    ...
    def rest_update_privilege(self, auth_token, model, pk, user_pk, privilege, HTTP_USER_AGENT, REMOTE_ADDR):
        ...
        return self.client.put(
            f"/api/{model}/{pk}/privileges/{user_pk}/",
            json.dumps(privilege, default=custom_json_handler),
            content_type="application/json",
            HTTP_USER_AGENT=HTTP_USER_AGENT,
            REMOTE_ADDR=REMOTE_ADDR,
        )
...
class ChangeSetMixin:
    ...
    def rest_get_paginated_changesets(self, auth_token, model, pk, HTTP_USER_AGENT, REMOTE_ADDR):
        ...
        return self.client.get(
            f"/api/{model}/{pk}/history_paginated/",
            HTTP_USER_AGENT=HTTP_USER_AGENT,
            REMOTE_ADDR=REMOTE_ADDR,
        )
...
class MeMixin:
    ...
    def rest_get_me(
        ...
        http_user_agent=HTTP_INFO["HTTP_USER_AGENT"],
        remote_addr=HTTP_INFO["REMOTE_ADDR"],
    ):
        self.client.credentials(HTTP_AUTHORIZATION="Token " + auth_token)
        response = self.client.get("/api/me/", HTTP_USER_AGENT=http_user_agent, REMOTE_ADDR=remote_addr)
    ...
    def rest_put_me(
        ...
        response = self.client.put(
            "/api/me/", data, content_type="application/json", HTTP_USER_AGENT=http_user_agent, REMOTE_ADDR=remote_addr
        )
        self.assertEqual(response.status_code, assert_status, response.content.decode())
        return response
...
class UserMixin:
    ...
    def rest_get_users(self, auth_token, HTTP_USER_AGENT, REMOTE_ADDR):
        ...
        return self.client.get("/api/users/", {}, HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR)

    def rest_get_user_with_pk(self, auth_token, user_pk, HTTP_USER_AGENT, REMOTE_ADDR):
        """Wrapper for getting a specific project via REST API"""
        self.client.credentials(HTTP_AUTHORIZATION="Token " + auth_token)

        return self.client.get(f"/api/users/{user_pk}/", {}, HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR)
    ...
    def rest_search_for_users(self, auth_token, search_query, HTTP_USER_AGENT, REMOTE_ADDR):
        ...
        return self.client.get(
            f"/api/users/?search={search_query}", {}, HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR
        )
...
class AuthenticationMixin:
    ...
    def create_student_role(self):
        ...
        RolePermissionAssignment.objects.create(role=role, permission=view_project_permission)
        # get view_resource permission
        view_resource_permission = Permission.objects.filter(
            codename="view_resource", content_type=Resource.get_content_type()
        ).first()
        # and assign it to this role
        RolePermissionAssignment.objects.create(role=role, permission=view_resource_permission)
        return role
    ...
    def create_strict_observer_role(self):
        ...
        RolePermissionAssignment.objects.bulk_create(
            [RolePermissionAssignment(role=role, permission=perm) for perm in perms]
        )
...
class ProjectsMixin:
    ...
    def create_project(
        self, auth_token, project_name, project_description, project_state, HTTP_USER_AGENT, REMOTE_ADDR
    ):
        """
        ...
        response = self.rest_create_project(
            auth_token, project_name, project_description, project_state, HTTP_USER_AGENT, REMOTE_ADDR
        )
        ...
        self.assertTrue(
            '"description":"' + project_description + '"' in content, msg="correct project description in response"
        )
    ...
    def validate_create_project(
        ...
        project = self.create_project(
            token, project_name, project_description, Project.INITIALIZED, HTTP_USER_AGENT, REMOTE_ADDR
        )
        ...
        self.assertEqual(
            has_default_role.count(), 1, msg="Validate that the user has the default role on project create"
        )
        ...
        for item in decoded:
            self.assertTrue(
                "created_by" in item, msg="projects endpoint returned a json object with created_by in the project"
            )
            self.assertTrue("name" in item, msg="projects endpoint returned a json object with name in the project")
            self.assertTrue(
                "project_state" in item,
                msg="projects endpoint returned a json object with project_state in the project",
            )
            self.assertTrue(
                "description" in item, msg="projects endpoint returned a json object with description in the project"
            )
    ...
    def rest_get_project(self, auth_token, project_pk, HTTP_USER_AGENT, REMOTE_ADDR):
        ...
        return self.client.get(
            f"/api/projects/{project_pk}/", {}, HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR
        )
    ...
    def rest_get_projects(self, auth_token, HTTP_USER_AGENT, REMOTE_ADDR):
        ...
        return self.client.get("/api/projects/", {}, HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR)
    ...
    def rest_create_project(
        ...
        return self.client.post(
            "/api/projects/",
            {"name": project_name, "description": project_description, "project_state": project_state},
            HTTP_USER_AGENT=HTTP_USER_AGENT,
            REMOTE_ADDR=REMOTE_ADDR,
        )

    def rest_edit_project(
        self, auth_token, project_pk, project_name, project_description, project_state, HTTP_USER_AGENT, REMOTE_ADDR
    ):
        """Wrapper for editing a project via REST API"""
        self.client.credentials(HTTP_AUTHORIZATION="Token " + auth_token)

        return self.client.put(
            f"/api/projects/{project_pk}/",
            {"name": project_name, "description": project_description, "project_state": project_state},
            HTTP_USER_AGENT=HTTP_USER_AGENT,
            REMOTE_ADDR=REMOTE_ADDR,
        )
    ...
    def rest_get_user_project_assignments(self, auth_token, project, HTTP_USER_AGENT, REMOTE_ADDR):
        ...
        return self.client.get(
            f"/api/projects/{project.pk}/acls/",
            {},
            HTTP_USER_AGENT=HTTP_USER_AGENT,
            REMOTE_ADDR=REMOTE_ADDR,
        )
    ...
    def rest_get_user_project_assignment(self, auth_token, project, user, HTTP_USER_AGENT, REMOTE_ADDR):
        ...
        # Make a REST API call for creating a new project user role assignment entry
        return self.client.get(
            f"/api/projects/{project.pk}/acls/?user={user.pk}",
            {},
            HTTP_USER_AGENT=HTTP_USER_AGENT,
            REMOTE_ADDR=REMOTE_ADDR,
        )
    ...
    def rest_edit_user_project_assignment(
        self, auth_token, project, assignment_pk, user, role, HTTP_USER_AGENT, REMOTE_ADDR
    ):
        ...
        return self.client.put(
            f"/api/projects/{project.pk}/acls/{assignment_pk}/",
            {
                "user_pk": user.pk,
                "role_pk": role.pk,
            },
    ...
    def rest_delete_user_from_project(self, auth_token, project_pk, assignment_pk, HTTP_USER_AGENT, REMOTE_ADDR):
        ...
        return self.client.delete(
            f"/api/projects/{project_pk}/acls/{assignment_pk}/",
            {},
            HTTP_USER_AGENT=HTTP_USER_AGENT,
            REMOTE_ADDR=REMOTE_ADDR,
        )
    ...
    def edit_project(
        ...
        response = self.rest_edit_project(
            auth_token, project_pk, project_name, project_description, project_state, HTTP_USER_AGENT, REMOTE_ADDR
        )
    ...
    def rest_duplicate_project(self, auth_token, project_pk, HTTP_USER_AGENT, REMOTE_ADDR):
        ...
        return self.client.post(
            f"/api/projects/{project_pk}/duplicate/",
            {},
            HTTP_USER_AGENT=HTTP_USER_AGENT,
            REMOTE_ADDR=REMOTE_ADDR,
        )
    ...
    def rest_trash_project(self, auth_token, project_pk, HTTP_USER_AGENT, REMOTE_ADDR):
        ...
        return self.client.patch(
            f"/api/projects/{project_pk}/soft_delete/",
            HTTP_USER_AGENT=HTTP_USER_AGENT,
            REMOTE_ADDR=REMOTE_ADDR,
        )
...
class ResourceMixin(TestLockMixin):
    ...
    def rest_get_resource_export_link(self, auth_token, task_pk, HTTP_USER_AGENT, REMOTE_ADDR):
        ...
        return self.client.get(
            f"/api/resources/{task_pk}/get_export_link/",
            HTTP_USER_AGENT=HTTP_USER_AGENT,
            REMOTE_ADDR=REMOTE_ADDR,
        )
    ...
    def rest_search_resources(self, auth_token, search_string, HTTP_USER_AGENT, REMOTE_ADDR):
        ...
        return self.client.get(
            f"/api/resources/?search={search_string}", HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR
        )
    ...
    def rest_get_resources_recently_modified_by_me(self, auth_token, number_of_days, HTTP_USER_AGENT, REMOTE_ADDR):
        ...
        return self.client.get(
            f"/api/resources/?recently_modified_by_me={number_of_days}",
            HTTP_USER_AGENT=HTTP_USER_AGENT,
            REMOTE_ADDR=REMOTE_ADDR,
        )
    ...
    def rest_get_resource(self, auth_token, resource_pk, HTTP_USER_AGENT, REMOTE_ADDR):
        ...
        return self.client.get(
            f"/api/resources/{resource_pk}/", HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR
        )

    def rest_query_resources(self, auth_token, HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR, **kwargs):
        """
    ...
    def rest_get_resources(self, auth_token, HTTP_USER_AGENT, REMOTE_ADDR):
        ...
        return self.client.get("/api/resources/", {}, HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR)
    ...
    def rest_get_resources_for_project(self, auth_token, project_pk, HTTP_USER_AGENT, REMOTE_ADDR):
        ...
        return self.client.get(
            "/api/resources/", {"project": project_pk}, HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR
        )
    ...
    def rest_export_resources(self, auth_token, HTTP_USER_AGENT, REMOTE_ADDR):
        ...
        """
        return self.client.get("/api/resources/export/", HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR)
    ...
    def rest_create_resource(
        ...
        **kwargs,
        ...
        if terms_of_use_pdf:
            ...
            fp = open(os.path.join(os.path.dirname(__file__), "demo_files", terms_of_use_pdf), "rb")
    ...
    def rest_restore_resource(self, auth_token, resource_pk, HTTP_USER_AGENT, REMOTE_ADDR):
        ...
        return self.client.patch(
            f"/api/resources/{resource_pk}/restore/",
            HTTP_USER_AGENT=HTTP_USER_AGENT,
            REMOTE_ADDR=REMOTE_ADDR,
        )
    ...
    def rest_trash_resource(self, auth_token, resource_pk, HTTP_USER_AGENT, REMOTE_ADDR):
        ...
        return self.client.patch(
            f"/api/resources/{resource_pk}/soft_delete/",
            HTTP_USER_AGENT=HTTP_USER_AGENT,
            REMOTE_ADDR=REMOTE_ADDR,
        )
