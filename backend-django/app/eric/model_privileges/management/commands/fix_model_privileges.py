...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            dest='list',
            default=True,
            help='List all entities with invalid permissions'
        )

        parser.add_argument(
            'entity_name',
            type=str,
            help='The entity that needs to be checked (e.g., Task)'
        )
        ...
        parser.add_argument(
            ...
            type=int,
            nargs='?',
            help='User Id that should be used for entity permission assignment'
        )
    ...
    def handle(self, *args, **options):
        ...
        if cts.exists():
            if cts.count() == 1:
                print("Found entity {entity_name}".format(entity_name=cts[0]))
                ct = cts[0]
            else:
        ...
        if options['list']:
            ...
            objects = model.objects.annotate(
                number_model_privileges=Count('model_privileges')
            ).filter(number_model_privileges=0)

            print("{ct}: {num_objects} of {total_count_objects} found without entity permission".format(
            ...
            if options['user_id']:
                ...
                print("Assigning those objects to user {}".format(user))
                ...
                print("{ct}: {num_objects} (should be 0) of {total_count_objects} found without entity permission".
                      format(ct=ct, num_objects=objects.count(), total_count_objects=total_count))
