...
from eric.core.models.abstract import (
    ...
    get_all_workbench_models_with_args,
...
from eric.metadata.rest.errors import InvalidFieldInputError, InvalidOperatorError, SearchParameterError
from eric.metadata.rest.queryset_filters import MetadataQuerySetFilter
from eric.metadata.rest.serializers import MetadataFieldSerializer, MetadataSerializer, MetadataTagSerializer
...
class MetadataViewSet(BaseAuthenticatedModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        data = self.fill_request_data_with_parent_entity(request.data, args, kwargs)
        return super().update(request, force_request_data=data, *args, **kwargs)
...
class MetadataFieldViewSet(BaseAuthenticatedModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if user.has_perm("metadata.add_metadatafield"):
            ...
            return Response(status=HTTP_200_OK, data={"request_status": "sent"})
...
class MetadataSearchViewSet(CreateModelMixin, GenericViewSet):
    ...
    def build_queryset_for_parameter(self, param):
        ...
        if field is None:
            raise SearchParameterError(parameter_index, _("Invalid field: {field_pk}").format(field_pk=field_pk))
    ...
    def extract_viewable_entities_from_queryset(self, metadata_queryset):
        ...
        for ct in pks_by_ct:
            pks = pks_by_ct[ct]
            if len(pks) > 0:
                prefetched_objects_by_ct[ct] = model_by_ct[ct].objects.viewable().filter(pk__in=pks).prefetch_common()
...
class MetadataTagViewSet(BaseAuthenticatedModelViewSet):
    """ViewSet for metadata tags"""

    serializer_class = MetadataTagSerializer
    queryset = MetadataTag.objects.all()
    pagination_class = None

    def create(self, request, *args, **kwargs):

        # if the tag already exists then just return it
        tags = MetadataTag.objects.filter(name__iexact=request.data.get("name"))
        if tags.count():
            tag = tags.first()
