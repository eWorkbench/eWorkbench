...
class MetadataField(BaseModel, CreatedModifiedByMixIn):
    ...
    def get_display_name_for_base_type(api_name):
        return [
            display_name
            for (internal_name, display_name) in MetadataField.BASE_TYPE_CHOICES
    ...
    def get_default_value(self):
        ...
        elif base_type == self.BASE_TYPE_TAG:
            return {"values": []}
...
class Metadata(BaseModel, CreatedModifiedByMixIn, OrderingModelMixin):
    ...
    def export_all_from_entity(entity):
        ...
        return [metadata.export() for metadata in Metadata.objects.filter(entity_id=entity.pk)]
