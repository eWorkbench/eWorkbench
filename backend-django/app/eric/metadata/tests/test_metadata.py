...
def api_data(field, values):
    return {"field": field.pk if field is not None else None, "values": values}


@unittest.skip("Metadata sub-endpoint is disabled for now")
class MetadataAPITest(APITestCase, AuthenticationMixin, ModelPrivilegeMixin, HelperMixin, ProjectsMixin, TaskMixin):
    ...
    def test_post_and_get_list(self):
        ...
        self.request_builder.post(self.task.pk, api_data(self.fraction_field, {"numerator": 7, "denominator": 123}))
        ...
        self.request_builder.post(self.task.pk, api_data(self.decimal_field, {"value": 987654321.012356879}))
        self.request_builder.post(self.task.pk, api_data(self.decimal_field, {"value": 3.14}))
    ...
    def test_patch(self):
        # post a metadata value
        response = self.request_builder.post(self.task.pk, api_data(self.decimal_field, {"value": 1.2}))
