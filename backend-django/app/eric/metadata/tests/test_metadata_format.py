...
from eric.metadata.utils import NumberFormatter, MetadataFormatter
...
class MetadataFormatTest(APITestCase, AuthenticationMixin, HelperMixin):
    ...
    def test_boolean_formatter(self):
        field = MetadataField.objects.create(
            name='MyCheckbox', description="...",
            base_type=MetadataField.BASE_TYPE_CHECKBOX, type_settings={},
        )
        self.superuser, self.token = self.create_user_and_login("superuser", is_superuser=True)

        metadata = Metadata.objects.create(field=field, entity=field, values={
            'value': True
        })
