...
class MetadataIntegrationTest(
    APITestCase, AuthenticationMixin, ModelPrivilegeMixin, HelperMixin, ProjectsMixin, TaskMixin
):
    def setUp(self):
        ...
        self.project = self.create_project(token, "MyProject", "My test project", Project.INITIALIZED, **http_info)
    ...
    def test_task_update_mixed(self):
        ...
        value = next(
            metadata["values"]["value"] for metadata in response_metadata2 if metadata["pk"] == value_to_pk_map[1.0]
        )
        self.assertEqual(value, 1.0)

        # check 2.0 is modified
        value = next(
            metadata["values"]["value"] for metadata in response_metadata2 if metadata["pk"] == value_to_pk_map[2.0]
        )
    ...
    def test_task_update_single_changes(self):
        ...
        data2 = {"metadata": response_metadata1}
        ...
        # add more data
        data3 = {"metadata": response_metadata2}
        ...
        # change some data
        data4 = {"metadata": response_metadata3}
        ...
        for metadata in response_metadata4:
            if metadata["pk"] == modified_metadata["pk"]:
                self.assertEqual(metadata["values"]["value"], new_value)
    ...
    def __get_metadata_of_task(self):
        response = self.client.get(path=f"/api/tasks/{self.task.pk}/", **http_info)
        return self.__check_response_and_get_metadata(response)

    def __send_patch_request(self, data):
        response = self.client.patch(
            path=f"/api/tasks/{self.task.pk}/",
            data=json.dumps(data, default=custom_json_handler),
            content_type="application/json",
            **http_info,
        )
