...
from eric.core.models.abstract import ChangeSetMixIn, ImportedDSSMixin, SoftDeleteMixin, WorkbenchEntityMixin
from eric.dss.config import DSS_MOUNT_PATH
from eric.dss.models.managers import DSSContainerManager, DSSEnvelopeManager, DSSFilesToImportManager
...
dss_storage = FileSystemStorage(location=settings.DSS_SETTINGS["MOUNT_PATH"], base_url="/api/dssfiles")
...
def get_upload_to_path(instance, filename):
    if instance.is_dss_file:
        ...
        full_path = os.path.join(container.path, envelope.path, drive.title, full_directory_path, filename)
...
class DSSEnvelope(BaseModel, ChangeSetMixIn, RevisionModelMixin, ImportedDSSMixin):
    ...
    path = models.CharField(max_length=4096, verbose_name=_("Path of the DSS envelope"))

    metadata_file_content = models.JSONField(
        verbose_name=_("The JSON content of the metadata file within this envelope"), null=False, blank=True
    )
...
class DSSContainer(
    ...
    name = models.CharField(max_length=255, verbose_name=_("Name of the DSS container"))

    path = models.CharField(max_length=4096, verbose_name=_("Path of the DSS container"), unique=True)
...
class DSSFilesToImport(BaseModel):
    ...
    #   file:           part1.tar.gz
    path = models.CharField(max_length=4096, verbose_name=_("Path of the File to import"), unique=True, db_index=True)
    ...
    imported = models.BooleanField(
        verbose_name=_("Whether this DSS File has been imported"), default=False, db_index=True, blank=True
    )
    ...
    import_attempts = models.IntegerField(
        default=0, verbose_name=_("The number of times the DSS Import was attempted for this path")
    )
    ...
    def validate_path_is_within_dss_storage(path):
        ...
        if path_is_incorrect:
            raise ValidationError(
                {"path": ValidationError(_("The path does not seem to be within a DSS storage"), code="invalid")}
            )
