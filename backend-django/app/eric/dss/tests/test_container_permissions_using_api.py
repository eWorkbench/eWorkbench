...
class ApiContainerPermissionTest(
    APITestCase, AuthenticationMixin, DSSContainerMixin, DSSEnvelopeMixin, DriveMixin, FileMixin
):
    """
    ...
    def setUp(self):
        ...
        # create container
        self.container, response = self.create_dsscontainer_orm(
            self.token1,
            None,
            "mf29er-dss-0000",
        ...
        response = self.rest_update_dsscontainer(
            self.token1,
            self.container.pk,
            None,
            "mf29er-dss-0000",
        ...
        response = self.rest_get_dsscontainer(self.token1, self.container.pk, HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.read_write_only_new_container = json.loads(response.content.decode())
        self.assertEqual(self.read_write_only_new_container["read_write_setting"], DSSContainer.READ_WRITE_ONLY_NEW)
    ...
    def test_moving_drive_in_read_only_container_is_forbidden(self):
        ...
        """
        # create envelope-1
        envelope_1, response = self.create_dssenvelope_orm(
            self.token1, "envelope-1", METADATA_JSON, self.container.pk, False, HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

        # create envelope-2
        envelope_2, response = self.create_dssenvelope_orm(
            self.token1, "envelope-2", METADATA_JSON, self.container.pk, False, HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

        # create drive and assign to envelope-1
        drive, response = self.create_dss_drive_orm(
            self.token1, None, "Some title", envelope_1.pk, False, HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        ...
        self.assertEqual(container["read_write_setting"], DSSContainer.READ_ONLY)

        # try to move the drive to another envelope
        response = self.rest_update_dss_drive(
            self.token1, drive.pk, None, "Some title", envelope_2.pk, False, HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        response = json.loads(response.content.decode())
        self.assertEqual(
            response["envelope"][0], "It is not allowed to move storages within read-only DSS " "containers"
        )

    def test_create_new_structure_in_read_write_only_new_container_is_ok(self):
        # create envelope
        envelope, response = self.create_dssenvelope_orm(
            self.token1,
            "envelope",
            METADATA_JSON,
        ...
        drive, response = self.create_dss_drive_orm(
            self.token1, None, "Some title", envelope.pk, False, HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        ...
        response = self.rest_create_dss_file(
            self.token1,
            None,
            "Test Title",
            "Test Description",
    ...
    def test_update_new_envelope_in_read_write_only_new_container_is_ok(self):
        ...
        # update envelope
        response = self.rest_update_dssenvelope(
            self.token1,
            envelope.pk,
            "envelope",
            METADATA_JSON,
    ...
    def test_update_new_drive_in_read_write_only_new_container_is_ok(self):
        ...
        response = self.rest_update_dss_drive(
            self.token1, drive.pk, None, "Some edited title", envelope.pk, False, HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_update_new_directory_in_read_write_only_new_container_is_ok(self):
        ...
        # create a directory within the drive
        response = self.rest_drive_create_dss_directory(
            self.token1, str(drive.pk), "My Subdir", root_dir, False, HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        decoded_response = json.loads(response.content.decode())
        directory = Directory.objects.get(pk=decoded_response["pk"])
        ...
        response = self.rest_drive_update_directory(
            self.token1, str(drive.pk), directory.pk, "My edited Subdir", root_dir, HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        decoded_response = json.loads(response.content.decode())
        directory = Directory.objects.get(pk=decoded_response["pk"])
        self.assertEqual(
            directory.name, "My edited Subdir", msg="Verify that directory title was properly saved in database"
        )
    ...
    def test_update_imported_drive_in_read_write_only_new_container_is_forbidden(self):
        ...
        # create drive and assign to envelope
        drive, response = self.create_dss_drive_orm(
            self.token1, None, "Some title", envelope.pk, True, HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

        response = self.rest_update_dss_drive(
            self.token1, drive.pk, None, "Some edited title", envelope.pk, True, HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_418_IM_A_TEAPOT)
    ...
    def test_update_imported_directory_in_read_write_only_new_container_is_forbidden(self):
        ...
        # create a directory within the drive
        response = self.rest_drive_create_dss_directory(
            self.token1, str(drive.pk), "My Subdir", root_dir, True, HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        decoded_response = json.loads(response.content.decode())
        directory = Directory.objects.get(pk=decoded_response["pk"])
