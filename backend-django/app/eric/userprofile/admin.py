...
class UserAdmin(BaseUserAdmin):
    ...
    def user_actions(self, obj):
        ...
        return mark_safe(" ".join([export_user_data, anonymize_user]))
    ...
    def export_personal_data(request, user_pk, *args, **kwargs):
        logger.info(f"Starting personal data export for user {user_pk}")
        ...
        response = HttpResponse(template.render(context, request), content_type="application/html")

        logger.info(f"Finished personal data export in {time.perf_counter() - start_time} sec.")
        logger.info(f"Queries: {len(connection.queries)}")

        filename = f"personal-data-export-{user.username}.html"
    ...
    def anonymize_user(self, request, user_pk, *args, **kwargs):
        ...
        if request.method == "POST":
            try:
                ...
                self.message_user(
                    request, _("User %(user)s has been anonymized") % {"user": user}, level=messages.SUCCESS
                )
            except Exception as error:
                self.message_user(
                    request,
                    _("User %(user)s could not be anonymized: %(error)s") % {"user": user, "error": error},
        ...
        return TemplateResponse(request, "admin/userprofile/anonymize_user.html", context=context)
