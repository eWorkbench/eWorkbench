...
class PublicCSVExportView(APIView, ABC):
    ...
    def build_stream_response(self):
        ...
        streaming_content = (
            writer.writerow(row) for row in self.headers_and_rows
        )
...
@method_decorator(
    cache_page(DISPLAY_EXPORT_CACHE_SECONDS),
    'dispatch'
)
class DisplayCsvView(PublicCSVExportView):
    ...
    def csv_rows(self):
        ...
        return (
            self.row_for_room(resource) for resource in study_rooms
        )
    ...
    def row_for_room(self, resource: Resource):
        ...
        cells = [
            study_room.room_id, study_room.name
        ]
