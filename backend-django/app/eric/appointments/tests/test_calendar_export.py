...
class CalendarExportTest(APITestCase, CommonTestMixin, StudyRoomExportMixin, MeetingMixin, ResourceMixin):
    ...
    def test_headers_only_for_no_bookings(self):
        ...
        self.assertEqual(HEADERS_STR, response.content.decode().strip())
    ...
    def test_headers_only_for_no_bookings_within_the_next_two_weeks(self):
        ...
        self.create_booking(token, self.resource1, start=now - timedelta(days=3), end=now - timedelta(days=1))
        self.create_booking(
            token, self.resource2, start=now - timedelta(days=2), end=start_of_today - timedelta(hours=3)
        )

        # create bookings in the future
        self.create_booking(token, self.resource1, start=now + timedelta(days=15), end=now + timedelta(days=16))
        self.create_booking(
            token, self.resource2, start=now + timedelta(days=14) + timedelta(hours=3), end=now + timedelta(days=16)
        )
    ...
    def test_csv_export_data(self):
        ...
        * Resource names must be escaped to allow CSV parsing
        ...
        booking1 = self.create_booking(
            token, self.resource1, start=now - timedelta(days=3), end=now + timedelta(hours=1)
        )

        # full booking within next two weeks
        booking2 = self.create_booking(token, self.resource2, start=now, end=now + timedelta(days=1))

        # start within next two weeks
        booking3 = self.create_booking(
            token, self.resource2, start=now + timedelta(days=13), end=now + timedelta(days=17)
        )
        booking4 = self.create_booking(  # same time, different resource
            token, self.resource3, start=now + timedelta(days=13), end=now + timedelta(days=17)
        )
        ...
        expected_content = (
            HEADERS_STR
            + LINE_BREAK
            + f'{booking1.pk};"First ""Resource""";2020-10-27T10:00:00+00:00;2020-10-30T11:00:00+00:00'
            + LINE_BREAK
            + f"{booking2.pk};Second, Resource;2020-10-30T10:00:00+00:00;2020-10-31T10:00:00+00:00"
            + LINE_BREAK
            + f"{booking3.pk};Second, Resource;2020-11-12T10:00:00+00:00;2020-11-16T10:00:00+00:00"
            + LINE_BREAK
            + f'{booking4.pk};"Third; Resource";2020-11-12T10:00:00+00:00;2020-11-16T10:00:00+00:00'
            + LINE_BREAK
        )
        ...
        self.assertEqual(expected_content, response_content)
