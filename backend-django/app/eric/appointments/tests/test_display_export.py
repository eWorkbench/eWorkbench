...
class DisplayExportTest(APITestCase, CommonTestMixin, StudyRoomExportMixin, MeetingMixin, ResourceMixin):
    ...
    def test_that_there_are_empty_cells_if_there_are_not_enough_bookings(self):
        ...
        self.assertEqual(
            "101;First Room;;;;;;;02.12.2020;DESIGN_001" + LINE_BREAK + "202;Second Room;;;;;;;02.12.2020;999_TEST_999",
            rows,
        )
    ...
    def test_that_the_current_date_is_put_in_every_row(self):
        ...
        with time_machine.travel("2333-09-12 18:00"):
            ...
            rows = self.get_csv_without_first_row(response)
            self.assertEqual(
                "101;First Room;;;;;;;12.09.2333;Default" + LINE_BREAK + "202;Second Room;;;;;;;12.09.2333;Default",
                rows,
            )
        ...
        with time_machine.travel("2020-12-02 13:00"):
            ...
            rows = self.get_csv_without_first_row(response)
            self.assertEqual(
                "101;First Room;;;;;;;02.12.2020;Default" + LINE_BREAK + "202;Second Room;;;;;;;02.12.2020;Default",
                rows,
            )
    ...
    def test_that_the_export_only_contains_upcoming_bookings_of_today(self):
        ...
        self.assert_response_status(response, status.HTTP_200_OK)
        rows = self.get_csv_without_first_row(response)
        self.assertEqual("12345;My Study Room;;;;;;;03.12.2020;Default", rows)
    ...
    @time_machine.travel("2020-12-03 11:00")
    def test_deleted_bookings(self):
        """
        Tests a successful export.
        Tests that deleted bookings will not be exported.
        """

        site_preferences.bookings_per_room_in_study_room_booking_display_export = 2
        token = self.superuser_token

        room1 = self.create_study_room(token, name="Room Dos", room_id=10002, display_design=self.default_design)

        now = timezone.now()

        # create multiple upcoming bookings (more than the configuration allows) for the second room
        self.create_booking(token, room1, start=now + timedelta(hours=1), end=now + timedelta(hours=2), title="A")
        booking_b = self.create_booking(
            token, room1, start=now + timedelta(hours=2), end=now + timedelta(hours=3), title="B"
        )
        self.create_booking(token, room1, start=now + timedelta(hours=3), end=now + timedelta(hours=4), title="C")
        self.create_booking(token, room1, start=now + timedelta(hours=4), end=now + timedelta(hours=5), title="D")

        self.rest_trash_meeting(token, booking_b.pk, HTTP_USER_AGENT, REMOTE_ADDR)
        ...
        self.assertEqual(
            "10002;Room Dos;"
            + "12:00;13:00;False;A;Superuser;03.12.2020;"
            + "14:00;15:00;False;C;Superuser;03.12.2020;"
            + "03.12.2020;Default",
            rows,
        )
