...
class TestLabBookUseCase(APITestCase, PluginMixin, PluginInstanceMixin, LabBookMixin, CommonTestMixin):
    ...
    def test_use_case_plugin_in_labbook(self):
        ...
        labbook, response = self.create_labbook_orm(
            self.token1, project_pk=None, title="My LabBook", is_template=False, **HTTP_INFO
        )
        ...
        response = self.rest_add_labbook_element(
            ...
            child_object_content_type=plugin_instance_ct.pk,
            child_object_id=plugin_instance["pk"],
            position_x=0,
            position_y=0,
            width=5,
            height=3,
        ...
        self.rest_update_plugininstance(
            self.token1, pk=plugin_instance["pk"], picture_file_name="demo1.png", rawdata_file_name="demo1.json"
        )
        ...
        response = self.rest_update_plugininstance(
            self.token1, pk=plugin_instance["pk"], picture_file_name="demo2.png", rawdata_file_name="demo2.json"
        )
        self.assert_response_status(response, status.HTTP_200_OK)
