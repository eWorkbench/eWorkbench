...
class TestPluginInstanceCreatePermission(APITestCase, PluginInstanceMixin):
    def setUp(self):
        ...
        self.add_plugininstance_without_project_permission = Permission.objects.filter(
            codename="add_plugininstance_without_project", content_type=PluginInstance.get_content_type()
        ).first()
    ...
    def test_users_can_create_plugin_instances_with_access_only(self):
        ...
        # user1: User
        self.assert_can_create_instance_of_exact_plugins(self.token1, allowed_plugins={self.plugin1, self.plugin3})

        # user2: External
        self.assert_can_create_instance_of_exact_plugins(self.token2, allowed_plugins={self.plugin1, self.plugin5})

        # user3: Student
        self.assert_can_create_instance_of_exact_plugins(
            self.token3,
            allowed_plugins={
                self.plugin1,
                self.plugin2,
    ...
    def assert_can_create_instance_of_plugin(self, auth_token, plugin: Plugin):
        ...
        self.assertEqual(
            ...
            'Should be able to create plugin instance of plugin "{}" -- {}'.format(
                plugin.title, response.content.decode()
            ),
        )
    ...
    def assert_can_not_create_instance_of_plugin(self, auth_token, plugin: Plugin):
        ...
        self.assertEqual(
            ...
            f'Should NOT be able to create plugin "{plugin.title}" -- {response.content.decode()}',
        )
