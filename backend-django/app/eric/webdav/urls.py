...
from eric.webdav.resources2 import AuthFsDavView, MyDriveDavResource, MyDriveListResource, MyProjectListResource
...
urlpatterns = [
    ...
    re_path(
        r"^d/(?P<drive_title>[\w \-]+) \((?P<drive>[0-9a-f-]+)\)(?P<path>.*)$",
    ...
    re_path(
        r"^p/(?P<project_title>[\w \-]+) \((?P<project>[0-9a-f-]+)\)/$",
        AuthFsDavView.as_view(resource_class=MyDriveListResource, lock_class=DummyLock, acl_class=ReadOnlyAcl),
        name="project-webdav-list",
    ),
    # List all directories and files within the provided drive (of a project)
    re_path(
        r"^p/(?P<project_title>[\w \-]+) \((?P<project>[0-9a-f-]+)\)/(?P<drive_title>[\w \-]+) \((?P<drive>[0-9a-f-]+)\)(?P<path>.*)$",  # noqa
        AuthFsDavView.as_view(resource_class=MyDriveDavResource, lock_class=DummyLock, acl_class=FullAcl),
