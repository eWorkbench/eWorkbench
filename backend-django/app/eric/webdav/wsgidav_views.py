...
from eric.webdav.wsgidav_responses import (
...
from eric.webdav.wsgidav_utils import (
    WEBDAV_NSMAP,
    D,
...
class DavView(TemplateView):
    ...
    def dispatch(self, request, path, *args, **kwargs):
        ...
        if (
            ...
            self.xbody = kwargs["xbody"] = etree.XPathDocumentEvaluator(
                parse(request, etree.XMLParser(ns_clean=True, resolve_entities=True)), namespaces=WEBDAV_NSMAP
            )
    ...
    def get(self, request, path, head=False, *args, **kwargs):
        ...
        if self.resource.is_object:
            ...
            if etags and (self.resource.etag in (e.strip(" ").strip('"') for e in etags.split(","))):
    ...
    def copy(self, request, path, xbody):
        ...
        print(f"Copying {self.resource.displayname} located at {path} to ...")
        ...
        if get_prop_names:
            ...
            responses = [
                D.response(
                    D.href(url_join(self.base_url, child.get_escaped_path())),
                    D.propstat(
                        D.prop(*get_property_tag_list(child, *(get_prop if get_prop else child.ALL_PROPS))),
                        D.status("HTTP/1.1 200 OK"),
                    ),
                )
                for child in children
    ...
    def proppatch(self, request, path, xbody, *args, **kwargs):
        ...
        body = D.multistatus(
            D.response(
                ...
                *[D.propstat(D.status("HTTP/1.1 200 OK"), D.prop(el.tag)) for el in props],
            )
        )
        return self.build_xml_response(body, HttpResponseMultiStatus)
    ...
    def build_xml_response(self, tree=None, response_class=HttpResponse, **kwargs):
        if tree is not None:
            content = etree.tostring(
                tree, xml_declaration=True, pretty_print=self.xml_pretty_print, encoding=self.xml_encoding
            )
        else:
            content = b""
        return response_class(content, content_type='text/xml; charset="%s"' % self.xml_encoding, **kwargs)
