...
from eric.core.rest.viewsets import BaseAuthenticatedReadOnlyModelViewSet, DeletableViewSetMixIn, ExportableViewSetMixIn
from eric.kanban_boards.models import KanbanBoard, KanbanBoardColumnTaskAssignment
from eric.kanban_boards.models.models import KanbanBoardColumn, KanbanBoardUserFilterSetting, KanbanBoardUserSetting
from eric.kanban_boards.rest.filters import KanbanBoardFilter
from eric.kanban_boards.rest.serializers import (
    KanbanBoardColumnTaskAssignmentSerializer,
    KanbanBoardSerializer,
    KanbanBoardUserFilterSettingSerializer,
...
from eric.shared_elements.models import Comment, Task

logger = logging.getLogger("eric.kanban_boards")


class TaskKanbanBoardAssignmentsViewSet(BaseAuthenticatedReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            KanbanBoardColumnTaskAssignment.objects.viewable()
            .filter(task=self.parent_object)
...
class KanbanBoardViewSet(
    ...
    @action(
        detail=True,
        methods=["GET"],
        url_path="background_image_thumbnail.png",
...
class KanbanBoardColumnTaskAssignmentViewSet(BaseAuthenticatedModelViewSet):
    ...
    def create_many(self, *args, **kwargs):
        ...
        max_ordering_qs = (
            KanbanBoardColumnTaskAssignment.objects.filter(kanban_board_column__kanban_board=self.parent_object)
            .order_by("kanban_board_column")
            .values("kanban_board_column")
            .annotate(max_ordering=Max("ordering", output_field=FloatField()))
        )
    ...
    def create(self, request, *args, **kwargs):
        ...
        get_object_or_404(Task.objects.viewable(), pk=request.data["task_id"])
        ...
        if "ordering" not in request.data:
            ...
            max_ordering = KanbanBoardColumnTaskAssignment.objects.filter(kanban_board_column=column).aggregate(
    ...
    def update(self, request, *args, **kwargs):
        ...
        if "kanban_board_column" in request.data:
            # verify that the user has access to the kanban column they are trying to add the task to
            get_object_or_404(self.parent_object.kanban_board_columns.all(), pk=request.data["kanban_board_column"])
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        task_assignments = KanbanBoardColumnTaskAssignment.objects.filter(
            kanban_board_column_id=obj.kanban_board_column, ordering__gte=obj.ordering
        ).exclude(pk=obj.pk)
    ...
    def get_queryset(self, *args, **kwargs):
        ...
        assignments = (
            KanbanBoardColumnTaskAssignment.objects.viewable()
            .filter(kanban_board_column__kanban_board=self.parent_object.pk)
            .select_related("task", "kanban_board_column", "kanban_board_column__kanban_board")
            .prefetch_related(
                "task__projects",
                "task__created_by",
                "task__created_by__userprofile",
                "task__last_modified_by",
                "task__last_modified_by__userprofile",
                "task__assigned_users",
                "task__assigned_users__userprofile",
        ...
        related_left_comments = {
            str(x["pk"]): x["num_related_left_comments"]
            for x in Relation.objects.filter(
                Q(right_content_type=task_content_type_id, right_object_id__in=tasks)
                & Q(left_content_type=comment_content_type_id)
            )
            .order_by("right_object_id", "right_content_type")
            .values("right_object_id", "right_content_type")
            .annotate(num_related_left_comments=Count("left_object_id"), pk=F("right_object_id"))
            .values("pk", "num_related_left_comments")
        }

        related_right_comments = {
            str(x["pk"]): x["num_related_right_comments"]
            for x in Relation.objects.filter(
                Q(left_content_type=task_content_type_id, left_object_id__in=tasks)
                & Q(right_content_type=comment_content_type_id)
            )
            .order_by("left_object_id", "left_content_type")
            .values("left_object_id", "left_content_type")
            .annotate(num_related_right_comments=Count("right_object_id"), pk=F("left_object_id"))
            .values("pk", "num_related_right_comments")
        }

        # iterate over child elements and set the child_object
        for task_assignment in assignments:
            task_assignment.num_related_comments = related_left_comments.get(
                str(task_assignment.task.id), 0
            ) + related_right_comments.get(str(task_assignment.task.id), 0)
            task_assignment.num_relations = (
                task_assignment.task.relations.count() - task_assignment.num_related_comments
            )
    ...
    def get_object(self):
        ...
        num_related_comments = Relation.objects.filter(
            Q(
                Q(left_content_type_id=task_content_type_id, left_object_id=obj.task.id)
                & Q(right_content_type=comment_content_type_id)
            )
            | Q(
                Q(right_content_type_id=task_content_type_id, right_object_id=obj.task.id)
                & Q(left_content_type=comment_content_type_id)
            )
        ).count()
    ...
    def move_assignment(self, *args, **kwargs):
        ...
        if from_column != to_column:
            ...
            task_assignments = KanbanBoardColumnTaskAssignment.objects.filter(
                kanban_board_column_id=to_column, ordering__gte=to_index
            ).exclude(pk=assignment_pk)
            ...
            task_assignments = KanbanBoardColumnTaskAssignment.objects.filter(
                kanban_board_column_id=from_column, ordering__gte=from_index
            ).exclude(pk=assignment_pk)
            ...
            task_assignments = (
                KanbanBoardColumnTaskAssignment.objects.viewable()
                .filter(kanban_board_column_id=to_column)
                .exclude(pk=assignment_pk)
            )
            ...
            if from_index < to_index:
                ...
                task_assignments = task_assignments.filter(ordering__gte=from_index, ordering__lte=to_index)
            ...
            task_assignments = (
                KanbanBoardColumnTaskAssignment.objects.viewable()
                .filter(kanban_board_column_id=from_column)
                .order_by("ordering")
            )
                ...
                for a in task_assignments:
                    ...
                    if a.ordering != i:
                        ...
                        logger.debug(
                            ...
                            "Detected ordering of assignment is {}, but expected it to be {}".format(a.ordering, i)
                        )
        ...
        logger.debug(
            ...
            "Moving took {} seconds".format(self.parent_object.pk, (end - start))
        )
...
class KanbanBoardUserFilterSettingViewSet(BaseAuthenticatedModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            KanbanBoardUserFilterSetting.objects.viewable()
            .filter(kanban_board=self.parent_object)
...
class KanbanBoardUserSettingViewSet(BaseAuthenticatedModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            KanbanBoardUserSetting.objects.viewable()
            .filter(kanban_board=self.parent_object)
            .prefetch_related("kanban_board")
        )
