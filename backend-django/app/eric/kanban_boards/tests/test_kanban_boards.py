...
from eric.kanban_boards.tests.core import KanbanBoardMixin, KanbanBoardClient
...
class TestGenericsKanbanBoards(APITestCase, EntityChangeRelatedProjectTestMixin, KanbanBoardMixin, TaskMixin):
    ...
    def setUp(self):
        ...
        self.data = [{
            ...
            'project_pks': None,
            'board_type': KanbanBoard.KANBAN_BOARD_TYPE_PERSONAL,
            'kanban_board_columns': [],
        }]
    ...
    def test_check_auto_created_kanban_board_column_attributes(self):
        ...
        response = self.rest_create_kanbanboard(self.token1, self.project1.pk, "test 1",
                                                KanbanBoard.KANBAN_BOARD_TYPE_PROJECT, HTTP_USER_AGENT, REMOTE_ADDR)
    ...
    def test_add_kanban_board_column(self):
        ...
        # create a new kanban board
        response = self.rest_create_kanbanboard(self.token1, self.project1.pk, "test 1",
                                                KanbanBoard.KANBAN_BOARD_TYPE_PROJECT, HTTP_USER_AGENT,
                                                REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_201_CREATED)
        ...
        response = self.rest_update_kanbanboard_child_elements(self.token1, kanban_board_pk, kanban_board_columns,
    ...
    def test_change_kanban_board_column_attributes(self):
        ...
        response = self.rest_update_kanbanboard_child_elements(self.token1, kanban_board['pk'], kanban_board_columns,
                                                               HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_200_OK)
        ...
        self.assertEqual(kanban_board_columns[3]['title'], "Changed column name",
                         msg="changed column should have the expected name")
        ...
        self.assertEqual(kanban_board_columns[0]['title'], cached_auto_generated_column_new['title'],
        ...
        self.assertEqual(kanban_board_columns[1]['title'], cached_new_column['title'],
                         msg="column should have the expected name")
        ...
        self.assertEqual(kanban_board_columns[2]['title'], cached_auto_generated_column_prog['title'],
                         msg="column should have the expected name")
        ...
        self.assertEqual(kanban_board_columns[3]['title'], cached_auto_generated_column_done['title'],
                         msg="column should have the expected name")
    ...
    def test_delete_kanban_board_columns(self):
        ...
        new_column_state_prog = {"title": "New column PROG", "icon": "", "ordering": 5,
    ...
    def test_delete_kanban_board_with_column_and_task(self):
        ...
        # create some tasks
        response = self.rest_create_task(self.token1, self.project1.pk, "New Task 1", "Description...",
        ...
        response = self.rest_add_task_to_kanbanboard_column(self.token1, kanban_board_pk, kanban_board_column_new_pk,
                                                            task1_pk, HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_201_CREATED)
        decoded = json.loads(response.content.decode())

        # check if task was added to kanban column
        self.assertEqual(decoded['kanban_board_column'], kanban_board_column_new_pk,
                         msg="Check if the task is in the correct column")
        ...
        self.assertEquals(KanbanBoard.objects.all().count(), 1)
        ...
        self.assertEquals(KanbanBoard.objects.all().count(), 0)
    ...
    def test_add_tasks_to_kanban_board_columns(self):
        ...
        decoded = json.loads(response.content.decode())
        task1_pk = decoded['pk']
        response = self.rest_create_task(self.token1, self.project1.pk, "New Task 2", "Description...",
                                         Task.TASK_STATE_PROGRESS, Task.TASK_PRIORITY_HIGH, datetime.now(),
                                         datetime.now() + timedelta(days=30),
                                         self.user1.pk,
                                         HTTP_USER_AGENT, REMOTE_ADDR)
        decoded = json.loads(response.content.decode())
        task2_pk = decoded['pk']
        response = self.rest_create_task(self.token1, self.project1.pk, "New Task 3", "Description...",
                                         Task.TASK_STATE_NEW, Task.TASK_PRIORITY_HIGH, datetime.now(),
                                         datetime.now() + timedelta(days=30),
                                         self.user1.pk,
                                         HTTP_USER_AGENT, REMOTE_ADDR)
        decoded = json.loads(response.content.decode())
        task3_pk = decoded['pk']
        response = self.rest_create_task(self.token1, self.project1.pk, "New Task 4", "Description...",
                                         Task.TASK_STATE_DONE, Task.TASK_PRIORITY_HIGH, datetime.now(),
                                         datetime.now() + timedelta(days=30),
                                         self.user1.pk,
                                         HTTP_USER_AGENT, REMOTE_ADDR)
        decoded = json.loads(response.content.decode())
        task4_pk = decoded['pk']
        response = self.rest_create_task(self.token1, self.project1.pk, "New Task 5", "Description...",
                                         Task.TASK_STATE_DONE, Task.TASK_PRIORITY_HIGH, datetime.now(),
                                         datetime.now() + timedelta(days=30),
                                         self.user1.pk,
                                         HTTP_USER_AGENT, REMOTE_ADDR)
        ...
        data = [
            {
                'kanban_board_column': kanban_board_column_new_pk,
                'task_id': task2_pk
            },
            {
                'kanban_board_column': kanban_board_column_prog_pk,
                'task_id': task3_pk
            },
            {
                'kanban_board_column': kanban_board_column_done_pk,
                'task_id': task4_pk
            },
            {
                'kanban_board_column': kanban_board_column_done_pk,
        ...
        response = self.rest_add_tasks_to_kanbanboard_column(self.token1, kanban_board_pk, data, HTTP_USER_AGENT,
                                                             REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_200_OK)
        decoded = json.loads(response.content.decode())

        # check if tasks were added to kanban columns
        self.assertEquals(len(decoded), 5, msg="The kanban board should have 5 tasks")
        ...
        self.assertEqual(decoded[0]['ordering'], 0,
                         msg="Task ordering has to be 0")
        self.assertEqual(decoded[1]['ordering'], 1,
                         msg="Task ordering has to be 1")
        self.assertEqual(decoded[2]['ordering'], 0,
                         msg="Task ordering has to be 0")
        self.assertEqual(decoded[3]['ordering'], 0,
                         msg="Task ordering has to be 0")
        self.assertEqual(decoded[4]['ordering'], 1,
                         msg="Task ordering has to be 1")
        ...
        self.assertEqual(decoded[2]['task']['state'], Task.TASK_STATE_NEW,
                         msg="Task state should not be changed")

        # try to add task1 again to the kanban board into a different column (should not work)
        response = self.rest_add_task_to_kanbanboard_column(self.token1, kanban_board_pk, kanban_board_column_done_pk,
                                                            task1_pk, HTTP_USER_AGENT, REMOTE_ADDR)
    ...
    def test_move_tasks_between_kanban_columns(self):
        ...
        #   Task 1
        data = {
            'assignment_pk': task1_assignment_pk,
            'to_column': kanban_board_column_prog_pk,
            'to_index': 1
        }

        response = self.rest_move_tasks_between_kanbanboard_columns(self.token1, kanban_board_pk, data, HTTP_USER_AGENT,
                                                                    REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_200_OK)
        decoded = json.loads(response.content.decode())
        ...
        # verify task 3
        self.assertEqual(decoded[1]['ordering'], 0,
                         msg="Task ordering has to be 0")
        ...
        # verify task 1
        self.assertEqual(decoded[2]['ordering'], 1,
                         msg="Task ordering has to be 1")
        ...
        #   Task 2
        data = {
            'assignment_pk': task2_assignment_pk,
            'to_column': kanban_board_column_done_pk,
            'to_index': 0
        }
        ...
        self.assertEqual(decoded[1]['ordering'], 1,
                         msg="Task ordering has to be 1")
        self.assertEqual(decoded[2]['ordering'], 2,
                         msg="Task ordering has to be 2")

        # check if task2 has the ordering 1
        self.assertEqual(decoded[1]['task']['pk'], task2_pk,
                         msg="task2 is the task with the ordering of 1")
    ...
    def test_delete_task_from_kanban_board(self):
        ...
        response = self.rest_delete_task_from_kanbanboard(self.token1, kanban_board_pk, task1_assignment_pk,
                                                          HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_204_NO_CONTENT)
        ...
        self.assertEqual(len(decoded), 0,
                         msg="No tasks should be available in this kanban view")
    ...
    def test_check_canban_board_permissions(self):
        ...
        new_column = {"title": "New column", "ordering": 4,
                      "icon": "",
                      "color": "rgba(224,224,224,0.65)"}
        kanban_board_columns.append(new_column)
        ...
        self.assertEquals(response.status_code, status.HTTP_200_OK)
        decoded = json.loads(response.content.decode())
        self.assertEquals(len(decoded), 0, msg="Task 1 should be in zero kanban boards")
        ...
        self.assertEquals(response.status_code, status.HTTP_200_OK)
        decoded = json.loads(response.content.decode())
        self.assertEquals(len(decoded), 0, msg="Task 2 should be in zero kanban boards")
        ...
        self.assertEquals(response.status_code, status.HTTP_200_OK)
        decoded = json.loads(response.content.decode())
        self.assertEquals(len(decoded), 0, msg="Task 3 should be in zero kanban boards")

        # add one task to kanban column as user2 (should not work)
        response = self.rest_add_task_to_kanbanboard_column(self.token2, kanban_board_pk, kanban_board_column_new_pk,
                                                            task1_pk, HTTP_USER_AGENT, REMOTE_ADDR)
        ...
        response = self.rest_create_privilege(self.token1, "tasks", task1_pk, self.user2.pk, HTTP_USER_AGENT,
                                              REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_201_CREATED)
        ...
        response = self.rest_create_privilege(self.token1, "tasks", task2_pk, self.user2.pk, HTTP_USER_AGENT,
                                              REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_201_CREATED)
        ...
        response = self.rest_create_privilege(self.token1, "tasks", task3_pk, self.user2.pk, HTTP_USER_AGENT,
                                              REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_201_CREATED)
        ...
        decoded = json.loads(response.content.decode())
        self.assertEquals(len(decoded), 0, msg="The kanban board should have no tasks")
        ...
        response = self.rest_add_tasks_to_kanbanboard_column(self.token2, kanban_board_pk, data, HTTP_USER_AGENT,
                                                             REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_403_FORBIDDEN)
        ...
        self.assertEquals(response.status_code, status.HTTP_200_OK)
        decoded = json.loads(response.content.decode())
        self.assertEquals(len(decoded), 1, msg="Task 1 should be in one kanban boards")
        ...
        self.assertEquals(response.status_code, status.HTTP_200_OK)
        decoded = json.loads(response.content.decode())
        self.assertEquals(len(decoded), 0, msg="Task 1 should be in no kanban boards")

        # try to delete the task from the kanban board as user2 (should not work)
        response = self.rest_delete_task_from_kanbanboard(self.token2, kanban_board_pk, task1_assignment_pk,
                                                          HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_403_FORBIDDEN)
        ...
        response = self.rest_get_tasks_of_kanbanboard(self.token2, kanban_board_pk, HTTP_USER_AGENT, REMOTE_ADDR)
        decoded = json.loads(response.content.decode())
        self.assertEquals(len(decoded), 1, msg="The kanban board should have 1 task")
        ...
        decoded = json.loads(response.content.decode())
        kanban_board_columns = decoded['kanban_board_columns']
        self.assertEquals(len(decoded['kanban_board_columns']), 4, msg="The kanban board should have 4 columns")
        ...
        response = self.rest_add_task_to_kanbanboard_column(self.token2, kanban_board_pk, kanban_board_column_new_pk,
                                                            task2_pk, HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_201_CREATED)

        # check if task was added
        response = self.rest_get_tasks_of_kanbanboard(self.token2, kanban_board_pk, HTTP_USER_AGENT, REMOTE_ADDR)
        decoded = json.loads(response.content.decode())
        self.assertEquals(len(decoded), 2, msg="A new task was added to the kanban board")
        ...
        response = self.rest_get_tasks_of_kanbanboard(self.token2, kanban_board_pk, HTTP_USER_AGENT, REMOTE_ADDR)
        decoded = json.loads(response.content.decode())
        self.assertEquals(len(decoded), 1, msg="The task was deleted from the kanban board")
    ...
    def test_can_not_edit_task_although_it_is_in_kanban_board(self):
        ...
        response = self.rest_create_task(
            self.token1, None, "Some secret task", "Some secret description",
            Task.TASK_STATE_NEW, Task.TASK_PRIORITY_HIGH, datetime.now(), datetime.now() + timedelta(hours=1),
            self.user1.pk,
            HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEquals(response.status_code, status.HTTP_201_CREATED)
        ...
        response = self.rest_create_kanbanboard(self.token2, None, "test 1",
                                                KanbanBoard.KANBAN_BOARD_TYPE_PROJECT, HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_201_CREATED)
        ...
        data = [
            {
                'kanban_board_column': kanban_board_columns[0]['pk'],
                'task_id': task.pk
            }
        ]
        ...
        self.assertEquals(KanbanBoardColumnTaskAssignment.objects.all().count(), 0)
        ...
        self.assertEquals(len(decoded_response), 0)

        # give user1 the view and edit privilege of the kanban board (user2 created the kanban board)
        response = self.rest_create_privilege(self.token2, "kanbanboards", kanban_board_pk, self.user1.pk,
                                              HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_201_CREATED)
        ...
        self.assertEquals(KanbanBoardColumnTaskAssignment.objects.all().count(), 1)
        ...
        self.assertEquals(decoded_response[0]['pk'], str(task.pk))
        ...
        response = self.rest_update_task(
            self.token2, task.pk, None, "oops", "oh no", Task.TASK_STATE_NEW, Task.TASK_PRIORITY_HIGH,
            datetime.now(), datetime.now() + timedelta(hours=1), self.user1.pk, HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEquals(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_background_image_and_color(self):
        ...
        self.assertEquals(response['background_color'], initial_bg_color)
        ...
        self.assertEquals(response['background_color'], second_bg_color)
        self.assertIsNone(response['background_image_thumbnail'])
        self.assertIsNone(response['download_background_image'])
        self.assertIsNone(response['download_background_image_thumbnail'])
