...
from eric.shared_elements.models import logging, vobject
...
def get_or_create_caldav_item_for_meeting(meeting):
    caldav_item, created = CaldavItem.objects.get_or_create(
        meeting=meeting, defaults={"name": f"{meeting.pk}-autogenerated.ics"}
    )
...
def read_or_create_ical_event(caldav_item, meeting):
    ...
    if caldav_item and caldav_item.text:
        try:
            ...
            LOGGER.error(f"Failed to parse VEvent data of CalDavItem {caldav_item.pk}")
            LOGGER.error(e)
