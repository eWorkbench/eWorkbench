...
def get_default_caldav_item_uid():
    ...
    :return:
    """
    return f"{now()}-{uuid.uuid4()}@workbench"
...
class CaldavItem(CreatedModifiedByMixin):
    ...
    id = models.CharField(
        _("CalDav Item Unique Identifier"), max_length=255, primary_key=True, default=get_default_caldav_item_uid
    )

    # name of the ical file (should be unique)
    name = models.CharField(verbose_name=_("Name of the ICAL File"), max_length=255, unique=True)

    # the ical information for this item
    text = models.TextField(verbose_name=_("ICAL File Content"))

    meeting = models.OneToOneField("shared_elements.Meeting", on_delete=models.CASCADE, null=True)
    ...
    deleted_via_caldav_on = models.DateTimeField(
        verbose_name=_("Whether this item was deleted via CalDav (and when)"), default=None, null=True
    )
    deleted_via_caldav_by = UserForeignKey(
        verbose_name=_("The user that deleted the item via CalDav"), default=None, null=True, on_delete=models.SET_NULL
    )
