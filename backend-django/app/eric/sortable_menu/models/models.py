...
class MenuEntry(BaseModel, OrderingModelMixin, VisibleModelMixin):
    ...
    route = models.CharField(max_length=128, verbose_name=_("Route (State) of this menu entry"), blank=False)
    ...
    def __str__(self):
        return f"MenuEntry for route {self.route}"
...
class MenuEntryParameter(BaseModel):
    ...
    name = models.CharField(max_length=128, verbose_name=_("Name of the parameter"), blank=False)

    value = models.CharField(max_length=128, verbose_name=_("Value of the parameter"), blank=True)

    def __str__(self):
        return "MenuEntryParameter for menu entry {menu_entry_name}: {name}='{value}'".format(
            menu_entry_name=self.menu_entry.route, name=self.name, value=self.value
        )
