...
from eric.shared_elements.models import Project, Task
...
class CleanDatabaseTest(
    ...
    def setUp(self):
        ...
        self.http_info_sa_task1 = HttpInfo(
            auth_token=self.token1, user_agent=HTTP_USER_AGENT, remote_address=REMOTE_ADDR
        )
        ...
        self.standalone_task1_version = self.rest_post_version(
            "tasks", self.sa_task1_pk, self.sa_task1_versiondata, self.http_info_sa_task1
        )

        self.standalone_labbook1 = self.rest_create_labbook(
            self.token1, None, "Standalone Test Labbok", False, HTTP_USER_AGENT, REMOTE_ADDR
        )
        ...
        self.standalone_note1 = self.rest_create_note(
            self.token1, None, "This is a standalone note", "Still a standalone note", HTTP_USER_AGENT, REMOTE_ADDR
        )
        ...
        self.standalone_kanban1 = self.rest_create_kanbanboard(
            self.token1, None, "Standalone Kanban Board", HTTP_USER_AGENT, REMOTE_ADDR
        )

        self.standalone_picture1 = self.rest_create_picture(
            self.token1, None, "Standalone Picture", "demo1.png", HTTP_USER_AGENT, REMOTE_ADDR
        )
        ...
        self.standalone_dmp1 = self.rest_create_dmp(
            self.token1, None, "Test DMP Title", self.dmp_status, self.dmp_form.pk, HTTP_USER_AGENT, REMOTE_ADDR
        )

        self.standalone_drive1 = self.rest_create_drive(
            self.token1, None, "Standalone drive a - b", HTTP_USER_AGENT, REMOTE_ADDR
        )

        # create a project with elements assigned to it
        self.project1 = self.create_project(
            self.token1,
            "My Own Project (testuser1)",
            "Only testuser1 has access to this project",
            Project.STARTED,
            HTTP_USER_AGENT,
            REMOTE_ADDR,
        )
        ...
        self.http_info_project1_task1 = HttpInfo(
            auth_token=self.token1, user_agent=HTTP_USER_AGENT, remote_address=REMOTE_ADDR
        )
        self.project1_task1_pk = self.project1_task1.data["pk"]
        self.project1_task1_versiondata = VersionData(
            object_id=self.project1_task1_pk, summary="My project 1 task 1 version"
        ).as_dict()
        self.project1_task1_version = self.rest_post_version(
            "tasks", self.project1_task1_pk, self.project1_task1_versiondata, self.http_info_project1_task1
        )
        ...
        self.project1_labbook1 = self.rest_create_labbook(
            self.token1, self.project1.pk, "Standalone Test Labbok", False, HTTP_USER_AGENT, REMOTE_ADDR
        )

        self.project1_contact1 = self.rest_create_contact(
            self.token1, self.project1.pk, "MSC", "Jane", "Mnuchin", "jane@email.com", HTTP_USER_AGENT, REMOTE_ADDR
        )

        self.project1_file1 = self.rest_create_file(
            self.token1, self.project1.pk, "MyFile", "This is a file!", "MyFileName", 1024, HTTP_USER_AGENT, REMOTE_ADDR
        )
        ...
        self.project1_kanban1 = self.rest_create_kanbanboard(
            self.token1, self.project1.pk, "Project Kanban Board", HTTP_USER_AGENT, REMOTE_ADDR
        )

        self.project1_picture1 = self.rest_create_picture(
            self.token1, self.project1.pk, "Project Picture", "demo1.png", HTTP_USER_AGENT, REMOTE_ADDR
        )

        self.project1_dmp1 = self.rest_create_dmp(
            self.token1,
            self.project1.pk,
            "Test Project DMP Title",
            self.dmp_status,
            self.dmp_form.pk,
            HTTP_USER_AGENT,
            REMOTE_ADDR,
        ...
        self.project1_drive1 = self.rest_create_drive(
            self.token1, None, "Project drive a - b", HTTP_USER_AGENT, REMOTE_ADDR
        )
        ...
        self.http_info_sa_task2 = HttpInfo(
            auth_token=self.token2, user_agent=HTTP_USER_AGENT, remote_address=REMOTE_ADDR
        )
        ...
        self.standalone_task2_version = self.rest_post_version(
            "tasks", self.sa_task2_pk, self.sa_task2_versiondata, self.http_info_sa_task2
        )

        self.standalone_labbook2 = self.rest_create_labbook(
            self.token2, None, "Standalone Test Labbok", False, HTTP_USER_AGENT, REMOTE_ADDR
        )
        ...
        self.standalone_note2 = self.rest_create_note(
            self.token2, None, "This is a standalone note", "Still a standalone note", HTTP_USER_AGENT, REMOTE_ADDR
        )
        ...
        self.standalone_kanban2 = self.rest_create_kanbanboard(
            self.token2, None, "Standalone Kanban Board", HTTP_USER_AGENT, REMOTE_ADDR
        )

        self.standalone_picture2 = self.rest_create_picture(
            self.token2, None, "Standalone Picture", "demo1.png", HTTP_USER_AGENT, REMOTE_ADDR
        )

        self.standalone_dmp2 = self.rest_create_dmp(
            self.token2, None, "Test DMP Title", self.dmp_status, self.dmp_form.pk, HTTP_USER_AGENT, REMOTE_ADDR
        )

        self.standalone_drive2 = self.rest_create_drive(
            self.token2, None, "Standalone drive a - b", HTTP_USER_AGENT, REMOTE_ADDR
        )

        # create a project with elements assigned to it
        self.project2 = self.create_project(
            self.token2,
            "My Own Project (testuser2)",
            "Only testuser2 has access to this project",
            Project.STARTED,
            HTTP_USER_AGENT,
            REMOTE_ADDR,
        )
        ...
        self.http_info_project2_task1 = HttpInfo(
            auth_token=self.token2, user_agent=HTTP_USER_AGENT, remote_address=REMOTE_ADDR
        )
        ...
        self.project2_task1_version = self.rest_post_version(
            "tasks", self.project2_task1_pk, self.project2_task1_versiondata, self.http_info_project2_task1
        )
        ...
        self.project2_labbook1 = self.rest_create_labbook(
            self.token2, self.project2.pk, "Standalone Test Labbok", False, HTTP_USER_AGENT, REMOTE_ADDR
        )

        self.project2_contact1 = self.rest_create_contact(
            self.token2, self.project2.pk, "MSC", "Jane", "Mnuchin", "jane@email.com", HTTP_USER_AGENT, REMOTE_ADDR
        )

        self.project2_file1 = self.rest_create_file(
            self.token2, self.project2.pk, "MyFile", "This is a file!", "MyFileName", 1024, HTTP_USER_AGENT, REMOTE_ADDR
        )
        ...
        self.project2_kanban1 = self.rest_create_kanbanboard(
            self.token2, self.project2.pk, "Project Kanban Board", HTTP_USER_AGENT, REMOTE_ADDR
        )

        self.project2_picture1 = self.rest_create_picture(
            self.token2, self.project2.pk, "Project Picture", "demo1.png", HTTP_USER_AGENT, REMOTE_ADDR
        )
        ...
        self.project2_drive1 = self.rest_create_drive(
            self.token2, self.project2.pk, "Project drive a - b", HTTP_USER_AGENT, REMOTE_ADDR
        )
    ...
    def test_verify_database_not_cleaned_without_setting(self):
        ...
        response = self.client.post(
            "/api/clean_workbench_models/", HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR
        )
