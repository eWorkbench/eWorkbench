...
class LdapLoginTest(APITestCase, AuthenticationMixin):
    ...
    def setUp(self):
        ...
        setattr(
            ...
            {
                "o": [
                    {"value_regex": "^employee$", "group_name": "User"},
                    {"value_regex": "^external$", "group_name": "External"},
                ]
            },
        )
    ...
    def test_login_case_sensitive(self):
        ...
        self.assertEqual(User.objects.filter(username="normaluser").exists(), False)
        ...
        self.assertEqual(User.objects.filter(username="normaluser").exists(), True)
        self.assertEqual(User.objects.filter(username="NormalUser").exists(), False)
    ...
    def test_login_and_group_assignment(self):
        ...
        # verify that the two ldap users do not exist yet
        self.assertEqual(User.objects.filter(username="normaluser").exists(), False)
        self.assertEqual(User.objects.filter(username="externaluser").exists(), False)
        ...
        # now the user should exist
        self.assertEqual(User.objects.filter(username="externaluser").exists(), True)
