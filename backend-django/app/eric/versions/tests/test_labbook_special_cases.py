...
class LabBookVersionSpecialCasesTest(APITestCase, VersionRestMixin, ProjectsMixin, AuthenticationMixin,
                                     ModelPrivilegeMixin, HelperMixin, NoteMixin, LabBookMixin, TestLockMixin):
    ...
    def test_trashed_element_is_restored(self):
        ...
        self.assertEquals(True, get_json_content(response)['deleted'])
        ...
        self.assertEquals(False, get_json_content(response)['deleted'])
    ...
    def test_deleted_element_is_ignored(self):
        ...
        response = self.rest_get_note(self.token, self.note.pk, **http_info)
        self.assertEquals(response.status_code, HTTP_404_NOT_FOUND, response.content.decode())
        ...
        self.assertEquals(0, len(child_elements))
