...
class HelperMixin:
    ...
    def assign_user_to_project(self, token, user, project):
        ...
        self.assertEquals(HTTP_201_CREATED, response.status_code, "Couldn't assign user to project")

    def remove_user_from_project(self, token, user, project):
        response = self.rest_get_user_project_assignment(
            token, project.pk, user.pk, HTTP_USER_AGENT, REMOTE_ADDRESS)
        self.assertEquals(HTTP_200_OK, response.status_code, "Couldn't get user-project assignment")

        assignment = response.content.decode()
        assignment_pk = assignment['pk']

        response = self.rest_delete_user_from_project(
            token, project.pk, assignment_pk, HTTP_USER_AGENT, REMOTE_ADDRESS)
        self.assertEquals(HTTP_200_OK, response.status_code, "Couldn't remove user from project")
