...
from eric.shared_elements.models import File, Note, Task, disable_permission_checks
from eric.shared_elements.tests.core import (
    ...
    NoteMixin,
    TaskMixin,
    UserAttendsMeetingMixin,
)
from eric.versions.tests import (
    HTTP_USER_AGENT,
    REMOTE_ADDRESS,
    VersionData,
    get_json_content,
...
class GenericVersionIntegrationTest(
    ...
    def set_up_projects(self):
        self.project1 = self.create_project(
            self.token_su, "MyProject", "My test project", Project.INITIALIZED, **http_info
        )
        self.project2 = self.create_project(
            self.token_su, "MySecondProject", "My second project", Project.INITIALIZED, **http_info
        )
        self.project3 = self.create_project(
            self.token_su, "MyThirdProject", "My third project", Project.INITIALIZED, **http_info
        )

        # create a temporary project, which we will delete after creating the first version
        self.project_tmp = self.create_project(self.token_su, "tmp project", "tmp", Project.INITIALIZED, **http_info)

    def allow_model_access(self, endpoint, pk, user):
        self.set_model_privilege_for_user(self.token_su, endpoint, pk, user, full_access_privilege=ModelPrivilege.ALLOW)
...
class TaskVersionIntegrationTest(GenericVersionIntegrationTest, APITestCase, TaskMixin, ElementLabelMixin):
    ...
    def create_initial_model_instance(self):
        ...
        # add checklist items
        response = self.rest_update_task_checklist_items(self.token_su, task.pk, init["checklist"], **http_info)
        self.assertEqual(HTTP_200_OK, response.status_code, "Couldn't update checklist")
    ...
    def update_task_labels(self, task_pk, label_pks):
        data = {"labels": label_pks}
        response = self.rest_update_task_partial(self.token_su, task_pk, data, **http_info)
        self.assertEqual(HTTP_200_OK, response.status_code, "Couldn't update task labels")

    def create_label(self, name):
        response = self.rest_create_label(self.token_su, name, "rgba(0,0,0,0)", **http_info)
        self.assertEqual(HTTP_201_CREATED, response.status_code, "Couldn't create label")
    ...
    def modify_model_instance(self, model_pk):
        ...
        response = self.rest_update_task_checklist_items(self.token_su, model_pk, mod["checklist"], **http_info)
        self.assertEqual(HTTP_200_OK, response.status_code, "Couldn't update checklist")
    ...
    def assert_data_equal(self, expected, actual):
        ...
        self.assertEqual(expected["start_date"].astimezone().isoformat(), actual["start_date"])
        self.assertEqual(expected["due_date"].astimezone().isoformat(), actual["due_date"])
...
class ContactVersionIntegrationTest(GenericVersionIntegrationTest, APITestCase, ContactMixin):
    ...
    def assert_database_still_has_modified_values_after_preview(self, actual):
        self.assertEqual("modified first name", actual["first_name"])
        self.assertEqual("modified last name", actual["last_name"])
        self.assertEqual("modified title", actual["academic_title"])
        self.assertEqual("modified@mail.com", actual["email"])
        self.assertEqual("modified company", actual["company"])
        self.assertEqual("0", actual["phone"])
    ...
    def assert_response_has_initial_values(self, initial, actual):
        self.assertEqual(initial.first_name, actual["first_name"])
        self.assertEqual(initial.last_name, actual["last_name"])
        self.assertEqual(initial.academic_title, actual["academic_title"])
        self.assertEqual(initial.email, actual["email"])
        self.assertEqual(initial.company, actual["company"])
        self.assertEqual(initial.phone, actual["phone"])
...
class MeetingVersionIntegrationTest(
    GenericVersionIntegrationTest,
    APITestCase,
    MeetingMixin,
    UserAttendsMeetingMixin,
    ContactAttendsMeetingMixin,
    ...
    def create_initial_model_instance(self):
        ...
        self.contact1, response = self.create_contact_orm(
            ...
            email="c1@email.com",
            company="c1 company",
            phone="004366411111111",
            **http_info,
        )
        self.assertEqual(HTTP_201_CREATED, response.status_code, "Couldn't create contact 1")

        self.contact_tmp, response = self.create_contact_orm(
            auth_token=self.token_su,
            project_pk=[self.project1.pk],
            academic_title="Mag.",
            first_name="c2firstname",
            last_name="c2lastname",
            email="c2@email.com",
            company="c2company",
            phone="00436642222222",
            **http_info,
        )
        self.assertEqual(HTTP_201_CREATED, response.status_code, "Couldn't create contact 2")

        # create initial meeting
        return self.create_meeting_orm(
            auth_token=self.token_su,
            project_pk=[self.project1.pk, self.project_tmp.pk],
            title="init title",
            description="a meeting description",
            start_date=get_utc_datetime(2001, 1, 1),
            end_date=get_utc_datetime(2002, 2, 2),
            attending_users=[self.user1.pk, self.user2.pk],
            attending_contacts=[self.contact1.pk, self.contact_tmp.pk],
            **http_info,
        )
    ...
    def modify_model_instance(self, model_pk):
        ...
        self.assertEqual(response.status_code, HTTP_200_OK, "Couldn't trash attending contact")
        response = self.rest_delete_contact(self.token_su, self.contact_tmp.pk, **http_info)
        self.assertEqual(response.status_code, HTTP_204_NO_CONTENT, "Couldn't delete attending contact")

        response = self.update_attending_contacts(self.token_su, model_pk, [], **http_info)
        self.assertEqual(response.status_code, HTTP_200_OK, response.content.decode())

        response = self.update_attending_users(self.token_su, model_pk, [self.superuser.pk], **http_info)
        self.assertEqual(response.status_code, HTTP_200_OK, response.content.decode())
        ...
        return self.rest_update_meeting(
            ...
            start_date=get_utc_datetime(1903, 3, 3),
            end_date=get_utc_datetime(1904, 4, 4),
            **http_info,
        )

    def assert_database_still_has_modified_values_after_preview(self, actual):
        self.assertEqual("modified", actual["title"])
        self.assertEqual("modified", actual["text"])

        expected_start_date = get_utc_datetime(1903, 3, 3).astimezone().isoformat()
        self.assertEqual(expected_start_date, actual["date_time_start"])

        expected_end_date = get_utc_datetime(1904, 4, 4).astimezone().isoformat()
        self.assertEqual(expected_end_date, actual["date_time_end"])
    ...
    def assert_response_has_initial_values(self, initial, actual):
        self.assertEqual(initial.title, actual["title"])
        self.assertEqual(initial.text, actual["text"])
...
class NoteVersionIntegrationTest(GenericVersionIntegrationTest, APITestCase, NoteMixin):
    ...
    def assert_database_still_has_modified_values_after_preview(self, actual):
        self.assertEqual("modified", actual["subject"])
        self.assertEqual("modified", actual["content"])
    ...
    def assert_response_has_initial_values(self, initial, actual):
        self.assertEqual(initial.subject, actual["subject"])
        self.assertEqual(initial.content, actual["content"])
...
class PictureVersionIntegrationTest(GenericVersionIntegrationTest, APITestCase, PictureMixin):
    ...
    def create_initial_model_instance(self):
        picture, create_response = self.create_picture_orm(
            self.token_su, [self.project1.pk, self.project_tmp.pk], "My picture", "demo1.png", **http_info
        )
        self.assertEqual(create_response.status_code, HTTP_201_CREATED, "Couldn't create picture")

        response = self.rest_update_picture_rendered_image_file(self.token_su, picture.pk, "demo2.png", **http_info)
        self.assertEqual(response.status_code, HTTP_200_OK, "Couldn't update rendered image")

        response = self.rest_update_picture_shape_file(self.token_su, picture.pk, "demo1.json", **http_info)
        self.assertEqual(response.status_code, HTTP_200_OK, "Couldn't update shape")

        # store initially generated values
        response = self.load_model_from_api(picture.pk)
        self.assertEqual(response.status_code, HTTP_200_OK, "Couldn't load picture")
    ...
    def modify_model_instance(self, model_pk):
        response = self.rest_update_picture(
            self.token_su, model_pk, [self.project2.pk], "My new picture title", "demo2.png", **http_info
        )
        self.assertEqual(response.status_code, HTTP_200_OK, "Couldn't update picture")

        response = self.rest_update_picture_rendered_image_file(self.token_su, model_pk, "demo1.png", **http_info)
        self.assertEqual(response.status_code, HTTP_200_OK, "Couldn't update rendered image")

        response = self.rest_update_picture_shape_file(self.token_su, model_pk, "demo2.json", **http_info)
        self.assertEqual(response.status_code, HTTP_200_OK, "Couldn't update shape")
    ...
    def assert_database_still_has_modified_values_after_preview(self, actual):
        self.assertEqual("My new picture title", actual["title"])
        ...
        self.assert_url_equal_without_parameters(self.modified_download_shapes, actual["download_shapes"])
    ...
    def assert_response_has_initial_values(self, initial, actual):
        self.assertEqual(initial.title, actual["title"])
        self.assertEqual(initial.width, actual["width"])
        self.assertEqual(initial.height, actual["height"])
    ...
    def assert_url_equal_without_parameters(self, url1, url2):
        self.assertEqual(self.strip_url_parameters(url1), self.strip_url_parameters(url2))
...
class FileVersionIntegrationTest(GenericVersionIntegrationTest, APITestCase, FileMixin):
    ...
    def create_initial_model_instance(self):
        create_response = self.rest_create_file(
            self.token_su,
            [self.project1.pk, self.project_tmp.pk],
            "my file",
            "my description",
            "demo1.txt",
            pow(2, 20),
        ...
        self.assertEqual(create_response.status_code, HTTP_201_CREATED, "Couldn't create file")
        ...
        self.assertEqual(response.status_code, HTTP_200_OK, "Couldn't load file")
    ...
    def modify_model_instance(self, model_pk):
        response = self.rest_update_file(
            ...
            [self.project2.pk],
            "my new file title",
            "my new file description",
            "newfilename.txt",
            pow(2, 10),
            **http_info,
        )
        self.assertEqual(response.status_code, HTTP_200_OK, "Couldn't update file")

        directory_pk = None
        response = self.rest_update_file_set_directory(self.token_su, model_pk, directory_pk, **http_info)
        self.assertEqual(response.status_code, HTTP_200_OK, "Couldn't update file directory")
    ...
    def assert_database_still_has_modified_values_after_preview(self, actual):
        self.assertEqual("my new file title", actual["title"])
        self.assertEqual("my new file description", actual["description"])
        self.assertEqual(None, actual["directory_id"])
        # mime_type doesn't change
        self.assertEqual(pow(2, 10), actual["file_size"])
        # rest_update_file prefixes the filename with a random string -> check only the last part
        self.assertEqual("newfilename.txt", actual["name"][-15:])
        self.assertEqual(self.modified_download, actual["download"])
    ...
    def assert_response_has_initial_values(self, initial, actual):
        self.assertEqual(initial.name, actual["name"])
        self.assertEqual(initial.description, actual["description"])
        self.assertEqual(initial.directory_id, actual["directory_id"])
        self.assertEqual(initial.mime_type, actual["mime_type"])
        self.assertEqual(initial.file_size, actual["file_size"])
        self.assertEqual(self.initial_download, actual["download"])
    ...
    def assert_download_file_name(self, expected, actual_response_data):
        ...
        self.assertEqual(expected, actual_file_name_end)


class LabBookVersionIntegrationTest(
    GenericVersionIntegrationTest, APITestCase, LabBookMixin, NoteMixin, PictureMixin, FileMixin
):
    picture1 = None
    ...
    def create_initial_model_instance(self):
        ...
        lab_book, create_response = self.create_labbook_orm(self.token_su, projects, "my lab book", False, **http_info)
        self.assertEqual(create_response.status_code, HTTP_201_CREATED, "Couldn't create lab book")
        ...
        self.file1, response = self.create_file_orm(
            self.token_su, projects, "my first file", "my first file desc", "file1.txt", 512, **http_info
        )
        self.assertEqual(response.status_code, HTTP_201_CREATED)
        ...
        self.file2, response = self.create_file_orm(
            self.token_su, projects, "my second file", "my second file desc", "file2.txt", 1024, **http_info
        )
        self.assertEqual(response.status_code, HTTP_201_CREATED)
        ...
        self.note1, response = self.create_note_orm(
            self.token_su, projects, "my first note", "my first note content", **http_info
        )
        self.assertEqual(response.status_code, HTTP_201_CREATED)
        ...
        self.note2, response = self.create_note_orm(
            self.token_su, projects, "my second note", "my second note content", **http_info
        )
        self.assertEqual(response.status_code, HTTP_201_CREATED)
    ...
    def add_element_to_labbook(self, lab_book, element_content_type_id, element_id, x, y):
        response = self.rest_add_labbook_element(
            self.token_su, lab_book.pk, element_content_type_id, element_id, x, y, 100, 100, **http_info
        )
        self.assertEqual(response.status_code, HTTP_201_CREATED, "Couldn't add element to LabBook")
        return get_json_content(response)["pk"]

    def modify_model_instance(self, model_pk):
        projects = [self.project2.pk]

        update_response = self.rest_update_labbook(
            self.token_su, model_pk, projects, "modified title", True, **http_info
        )
        self.assertEqual(update_response.status_code, HTTP_200_OK)

        # --- pictures ---
        response = self.rest_update_picture(
            self.token_su, self.picture1.pk, projects, "modified pic title", "demo2.png", **http_info
        )
        ...
        response = self.rest_update_picture(
            self.token_su, self.picture2.pk, projects, "modified pic2 title", "demo2.png", **http_info
        )
        self.assertEqual(response.status_code, HTTP_200_OK)
        ...
        response = self.rest_update_note(
            self.token_su, self.note1.pk, projects, "mod note1 subject", "mod note1 content", **http_info
        )
        self.assertEqual(response.status_code, HTTP_200_OK)
        ...
        response = self.rest_update_note(
            self.token_su, self.note2.pk, projects, "mod note2 subject", "mod note2 content", **http_info
        )
        self.assertEqual(response.status_code, HTTP_200_OK)
        ...
        self.assertEqual(response.status_code, HTTP_204_NO_CONTENT, response.content.decode())

        # add an element
        note_content_type_id = Note.get_content_type().id
        self.tmp_note, response = self.create_note_orm(
            self.token_su, projects, "my tmp note", "my tmp note content", **http_info
        )
        self.assertEqual(response.status_code, HTTP_201_CREATED)
    ...
    def assert_database_still_has_modified_values_after_preview(self, actual):
        ...
        for actual_element in actual_elements:
            ...
            if object_id == str(self.picture2.pk):
                self.assertEqual(actual_element["position_x"], 0)
                self.assertEqual(actual_element["position_y"], 100)
            elif object_id == str(self.file1.pk):
                self.assertEqual(actual_element["position_x"], 0)
                self.assertEqual(actual_element["position_y"], 200)
            elif object_id == str(self.file2.pk):
                self.assertEqual(actual_element["position_x"], 0)
                self.assertEqual(actual_element["position_y"], 300)
            elif object_id == str(self.note1.pk):
                self.assertEqual(actual_element["position_x"], 0)
                self.assertEqual(actual_element["position_y"], 400)
            elif object_id == str(self.note2.pk):
                self.assertEqual(actual_element["position_x"], 0)
                self.assertEqual(actual_element["position_y"], 500)
            # tmp_note added
            elif object_id == str(self.tmp_note.pk):
                self.assertEqual(actual_element["position_x"], 42)
                self.assertEqual(actual_element["position_y"], 999)
            else:
    ...
    def assert_response_has_initial_values(self, initial, actual):
        ...
        self.assertEqual(initial.title, actual["title"])
        self.assertEqual(initial.description, actual["description"])
        self.assertEqual(initial.is_template, actual["is_template"])
    ...
    def assert_database_has_initial_values(self, initial, actual):
        ...
        response = self.rest_get_picture(self.token_su, self.picture1.pk, **http_info)
        json = get_json_content(response)
        self.assertEqual(json["title"], self.picture1.title)

        response = self.rest_get_picture(self.token_su, self.picture2.pk, **http_info)
        json = get_json_content(response)
        self.assertEqual(json["title"], self.picture2.title)

        response = self.rest_get_file(self.token_su, self.file1.pk, **http_info)
        json = get_json_content(response)
        self.assertEqual(json["title"], "my first file")

        response = self.rest_get_file(self.token_su, self.file2.pk, **http_info)
        json = get_json_content(response)
        self.assertEqual(json["title"], "my second file")

        response = self.rest_get_note(self.token_su, self.note1.pk, **http_info)
        json = get_json_content(response)
        self.assertEqual(json["subject"], self.note1.subject)

        response = self.rest_get_note(self.token_su, self.note2.pk, **http_info)
        json = get_json_content(response)
        self.assertEqual(json["subject"], self.note2.subject)
    ...
    def assert_initial_labbook_elements(self, model_pk):
        ...
        for actual_element in actual_elements:
            ...
            if object_id == str(self.picture1.pk):
                self.assertEqual(actual_element["position_x"], 0)
                self.assertEqual(actual_element["position_y"], 0)
    ...
    def assert_values_of_restore_response(self, initial_model, json):
        ...
        actual_child_elements = {normalize_child_from_json(c) for c in json["child_elements"]}
...
class DmpVersionIntegrationTest(GenericVersionIntegrationTest, APITestCase, DmpsMixin):
    ...
    def assert_expected_values(self, expected, actual):
        ...
        self.assertEqual(2, len(actual["dmp_form_data"]))
        self.assertEqual(actual["dmp_form_data"][0]["value"], expected["data1"])
        self.assertEqual(actual["dmp_form_data"][1]["value"], expected["data2"])
