...
class TestGenericsDrives(APITestCase, EntityChangeRelatedProjectTestMixin, DriveMixin, NoteMixin, FileMixin):
    ...
    def test_create_directory(self):
        ...
        response = self.rest_drive_create_directory(self.token1, str(drive.pk), "My Subdir", root_dir, HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_201_CREATED)

        decoded_response = json.loads(response.content.decode())
        directory = Directory.objects.get(pk=decoded_response['pk'])

        self.assertEquals(decoded_response['name'], "My Subdir", msg="Verify that directory title was properly transmitted via REST API")
        ...
        print(directories)
        self.assertEquals(len(directories), 2, msg="There should be two directories for this drive")
        self.assertEquals(directories[0]['drive_id'], str(drive.id), msg="Directories should be associated to this drive")
        self.assertEquals(directories[1]['drive_id'], str(drive.id), msg="Directories should be associated to this drive")
        ...
        response = self.rest_drive_create_directory(self.token1, str(second_drive.pk), "My Subdir", second_root_dir,
        ...
        self.assertEquals(response.status_code, status.HTTP_200_OK)
        directories = json.loads(response.content.decode())
        self.assertEquals(len(directories), 2, msg="There should be two directories for the second drive")
        self.assertEquals(directories[0]['drive_id'], str(second_drive.id),
                          msg="Directories should be associated to the second drive")
        self.assertEquals(directories[1]['drive_id'], str(second_drive.id),
                          msg="Directories should be associated to the second drive")
    ...
    def test_create_sub_directories(self):
        ...
        response = self.rest_drive_create_directory(self.token1, str(drive.pk), "subdir", decoded_directory['pk'], HTTP_USER_AGENT,
                                                    REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_201_CREATED)
        ...
        self.assertEquals(decoded_sub_directory['directory'], decoded_directory['pk'])

        # Create another sub directory (below the first directory)
        response = self.rest_drive_create_directory(self.token1, str(drive.pk), "second subdir", decoded_directory['pk'],
                                                    HTTP_USER_AGENT,
                                                    REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_201_CREATED)
        ...
        # check that the sub directory has the above directory as parent directory
        self.assertEquals(decoded_sub_directory2['directory'], decoded_directory['pk'])
    ...
    def test_update_sub_directory(self):
        ...
        response = self.rest_drive_update_directory(self.token1, str(drive.pk), decoded_directory['pk'],
                                                    "My Directory", None, HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_200_OK)

        # verify that the title has changed in database
        directory = Directory.objects.get(pk=decoded_directory['pk'])
        self.assertEquals(directory.name, "My Directory")
    ...
    def test_create_sub_directory_in_wrong_drive(self):
        ...
        response = self.rest_drive_create_directory(self.token1, str(first_drive.pk), "My dir", first_root_dir, HTTP_USER_AGENT,
                                                    REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_201_CREATED)
        ...
        # create a directory within the second drive
        response = self.rest_drive_create_directory(self.token1, str(second_drive.pk), "My dir", second_root_dir, HTTP_USER_AGENT,
                                                    REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_201_CREATED)
        ...
        response = self.rest_drive_create_directory(self.token1, str(first_drive.pk), "My subdir", second_drive_first_directory['pk'],
                                                    HTTP_USER_AGENT, REMOTE_ADDR)
        ...
        response = self.rest_drive_update_directory(self.token1, str(second_drive.pk), second_drive_first_directory['pk'], "My dir", first_drive_first_directory['pk'],
                                                    HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_can_not_set_parent_directory_to_own_directory(self):
        ...
        response = self.rest_drive_update_directory(self.token1, str(drive.pk), decoded_directory['pk'],
                                                    "My dir", decoded_directory['pk'], HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_can_not_update_directory_recursive_relation_between_subdirectories(self):
        ...
        response = self.rest_drive_create_directory(self.token1, str(drive.pk), "My subdir", decoded_directory['pk'], HTTP_USER_AGENT,
                                                    REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_201_CREATED)
        ...
        response = self.rest_drive_update_directory(self.token1, str(drive.pk), decoded_directory['pk'], "My dir", decoded_sub_directory['pk'],
                                                    HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_auto_create_directory_when_creating_drive(self):
        ...
        self.assertEquals(Drive.objects.all().count(), 0)
        self.assertEquals(Directory.objects.all().count(), 0)
        ...
        self.assertEquals(Drive.objects.all().count(), 1)
        self.assertEquals(Directory.objects.filter(drive=drive).count(), 1)
        self.assertEquals(Directory.objects.filter(drive=drive, is_virtual_root=True).count(), 1)
    ...
    def test_can_not_create_directories_with_same_name_on_same_level(self):
        ...
        response = self.rest_drive_create_directory(self.token1, str(drive.pk), "My dir", decoded_directory['pk'], HTTP_USER_AGENT,
                                                    REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_201_CREATED)
        ...
        response = self.rest_drive_create_directory(self.token1, str(drive.pk), "My dir", decoded_directory['pk'],
                                                    HTTP_USER_AGENT,
                                                    REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_remove_directory(self):
        ...
        response = self.rest_drive_delete_directory(self.token1, str(drive.pk), decoded_directory['pk'], HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_204_NO_CONTENT)

        # the directory should no longer exist
        self.assertFalse(Directory.objects.filter(pk=decoded_directory['pk']).exists(), msg="Directory should no longer exist")
    ...
    def test_remove_directory_also_trashes_files(self):
        ...
        # create a file
        response = self.rest_create_file(self.token1, None, "My file", "M description", "my_file.txt", 1024,
                                         HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_201_CREATED)
        decoded_file = json.loads(response.content.decode())

        # assign file to directory
        response = self.rest_update_file_set_directory(self.token1, decoded_file['pk'], decoded_directory['pk'],
                                                       HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_200_OK)

        # now delete the directory
        response = self.rest_drive_delete_directory(self.token1, str(drive.pk), decoded_directory['pk'],
                                                    HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_204_NO_CONTENT)
        ...
        self.assertEquals(file.deleted, True)
    ...
    def test_remove_directory_recursively(self):
        ...
        response = self.rest_drive_create_directory(self.token1, str(drive.pk), "My subdir", decoded_directory['pk'],
                                                    HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_can_not_remove_file_from_directory_if_directory_is_not_editable(self):
        ...
        response = self.rest_update_file_set_directory(self.token2, decoded_file['pk'], None,
                                                       HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_download_directory_as_zipfile(self):
        ...
        * Directory 'My Dir'
        ...
        # create a file
        response = self.rest_create_file(self.token1, None, "My file", "My description", "my_file.txt", 1024,
                                         HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_201_CREATED)
        decoded_file = json.loads(response.content.decode())
        ...
        # create another file
        response = self.rest_create_file(self.token1, None, "Another file", "Some other description", "another_file.txt", 1024,
                                         HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_201_CREATED)
        decoded_another_file = json.loads(response.content.decode())

        # assign another file to the sub directory
        response = self.rest_update_file_set_directory(self.token1, decoded_another_file['pk'], decoded_sub_directory['pk'],
                                                       HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_200_OK)

        # we should now have the desired structure - try to download the first directory "My Dir" (decoded_directory)
        response = self.rest_drive_download_directory(self.token1, str(drive.pk), decoded_directory['pk'], HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEquals(response.status_code, status.HTTP_200_OK)
        ...
        self.assertEquals(len(zipfile_names), 2)
    ...
    def test_drives_with_special_characters(self):
        ...
        decoded_response = json.loads(response.content.decode())
        self.assertEquals(len(decoded_response), 4)
