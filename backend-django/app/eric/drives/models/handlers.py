...
def ensure_only_one_root_directory(instance, *args, **kwargs):
    ...
    if instance.is_virtual_root:
        ...
        existing_roots = Directory.objects.filter(
            drive=instance.drive, is_virtual_root=True
        ).exclude(
            pk=instance.pk
        )
        ...
        if instance.name != '/':
            raise ValidationError({
                'name': ValidationError(
                    _("You can not rename the root folder"),
        ...
        raise ValidationError({
            'directory': ValidationError(
                _("You need to select a directory"),
...
def rename_file_increase_number(filename):
    ...
    if not matches or len(matches) == 0:
        ...
        new_filename = regex.sub("({})".format(cur_number), new_filename)
    ...
    try:
        if instance.directory.pk:
            ...
            while cnt < 10:
                ...
                files = File.objects.not_deleted().filter(
