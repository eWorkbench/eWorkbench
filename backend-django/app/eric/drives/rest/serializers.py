...
class DirectorySerializer(BaseModelWithCreatedByAndSoftDeleteSerializer):
    ...
    download_directory = serializers.SerializerMethodField(read_only=True)
    ...
    class Meta:
        ...
        read_only_fields = ("is_virtual_root",)
...
class DriveSerializer(BaseModelWithCreatedByAndSoftDeleteSerializer, EntityMetadataSerializerMixin):
    ...
    # url for file-list
    sub_directories_url = HyperlinkedToListField(view_name="drive-sub_directories-list", lookup_field_name="drive_pk")

    dss_envelope_id = serializers.PrimaryKeyRelatedField(
        queryset=DSSEnvelope.objects.all(), source="envelope", many=False, required=False, allow_null=True
    )

    envelope_path = serializers.StringRelatedField(source="envelope", many=False, required=False, allow_null=True)

    container_id = serializers.PrimaryKeyRelatedField(
        queryset=DSSContainer.objects.all(), source="envelope.container", many=False, required=False, allow_null=True
    )
    ...
    def get_webdav_url(self, obj):
        ...
        if not obj.is_dss_drive:
            return reverse(
                "webdav-drive", request=request, kwargs={"drive": obj.pk, "path": "/", "drive_title": stripped_title}
            )
        return None
