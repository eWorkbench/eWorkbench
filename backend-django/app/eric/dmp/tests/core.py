...
from eric.dmp.models import Dmp, DmpFormData, DmpFormField
...
class DmpsMixin:
    ...
    def rest_get_dmp(self, dmp_id, HTTP_USER_AGENT, REMOTE_ADDR):
        """GETs a dmp"""
        return self.client.get(path=f"/api/dmps/{dmp_id}/", HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR)
    ...
    def rest_get_dmps(self, auth_token, HTTP_USER_AGENT, REMOTE_ADDR):
        ...
        return self.client.get("/api/dmps/", {}, HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR)

    def rest_create_dmp(self, auth_token, projects, dmp_title, dmp_status, dmp_form_id, HTTP_USER_AGENT, REMOTE_ADDR):
        data = {"title": dmp_title, "status": dmp_status, "dmp_form": dmp_form_id}
        ...
        response = self.client.post("/api/dmps/", data, HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR)

        return response

    def create_dmp(self, auth_token, project_ids, dmp_title, dmp_status, dmp_form_id, HTTP_USER_AGENT, REMOTE_ADDR):
        """
        ...
        response = self.rest_create_dmp(
            auth_token, project_ids, dmp_title, dmp_status, dmp_form_id, HTTP_USER_AGENT, REMOTE_ADDR
        )
        ...
        self.assertEqual(Dmp.objects.all().count(), initial_dmp_length + 1, msg="check if the dmp was created")
        ...
        for dmp_form_field in dmp_form_fields:
            ...
            self.assertEqual(data.name, dmp_form_field.name, msg="check if the dmp form data has the correct name")
            self.assertEqual(data.type, dmp_form_field.type, msg="check if the dmp form data has the correct type")
            self.assertEqual(
                data.infotext, dmp_form_field.infotext, msg="check if the dmp form data has the correct infotext"
            )

        return dmp

    def create_dmp_orm(self, auth_token, project_ids, dmp_title, dmp_status, dmp_form_id, HTTP_USER_AGENT, REMOTE_ADDR):
    ...
    def update_dmp(
        self, auth_token, projects, dmp_title, dmp_status, dmp_id, dmp_form_id, HTTP_USER_AGENT, REMOTE_ADDR
    ):
        """
        ...
        response = self.client.put(
            f"/api/dmps/{dmp_id}/", data, HTTP_USER_AGENT=HTTP_USER_AGENT, REMOTE_ADDR=REMOTE_ADDR
        )
        return response

    def update_dmp_and_nested_dmp_form_data(
        self,
        auth_token,
        project_id,
        dmp_title,
        ...
        data = {
            ...
            "dmp_form_data": [{"pk": str(dmp_form_data_id), "value": dmp_form_data_value}],
        }
    ...
    def update_dmp_form_data(self, dmp_id, dmp_form_data_pk_and_value_list, HTTP_USER_AGENT, REMOTE_ADDR):

        data = {"dmp_form_data": dmp_form_data_pk_and_value_list}
