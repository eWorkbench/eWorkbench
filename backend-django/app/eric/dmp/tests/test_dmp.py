...
class DmpsTest(APITestCase, AuthenticationMixin, ProjectsMixin, DmpsMixin):
    ...
    def setUp(self):
        ...
        self.add_dmp_permission = Permission.objects.filter(
            codename="add_dmp", content_type=Dmp.get_content_type()
        ).first()

        self.add_dmp_without_project_permission = Permission.objects.filter(
            codename="add_dmp_without_project", content_type=Dmp.get_content_type()
        ).first()
        ...
        self.dmp_form_field1 = self.validate_create_dmp_form_field(
            "dmpformfield1", "NUM", "unittest dmpformfield1", self.dmp_form1
        )
        self.dmp_form_field2 = self.validate_create_dmp_form_field(
            "dmpformfield2", "TXA", "unittest dmpformfield2", self.dmp_form1
        )

        # create 3 dmp form field which are associated to dmp_form2
        self.dmp_form_field3 = self.validate_create_dmp_form_field(
            "dmpformfield3", "TXF", "unittest dmpformfield3", self.dmp_form2
        )
        self.dmp_form_field4 = self.validate_create_dmp_form_field(
            "dmpformfield4", "TXA", "unittest dmpformfield4", self.dmp_form2
        )
        self.dmp_form_field5 = self.validate_create_dmp_form_field(
            "dmpformfield5", "TXA", "unittest dmpformfield5", self.dmp_form2
        )
    ...
    def validate_create_dmp_forms(self, title, description):
        ...
        self.assertEqual(
            len(DmpForm.objects.all()), initial_dmp_form_length + 1, msg="check if the dmp form was created"
        )
    ...
    def validate_create_dmp_form_field(self, name, type, infotext, form_field):
        ...
        dmp_form_field = DmpFormField.objects.create(name=name, type=type, infotext=infotext, dmp_form=form_field)

        # check if the dmp form field object was created
        self.assertEqual(
            len(DmpFormField.objects.all()),
            initial_dmp_form_field_length + 1,
    ...
    def test_create_dmp(self):
        ...
        dmp = self.create_dmp(
            self.token1, self.project.pk, "First DMP", Dmp.NEW, self.dmp_form1.pk, HTTP_USER_AGENT, REMOTE_ADDR
        )
        # verify that a dmp was saved in the database
        self.assertEqual(Dmp.objects.all().count(), 1)
        self.assertEqual(dmp.title, "First DMP")
        self.assertEqual(dmp.status, Dmp.NEW)

        # verify that two DMP Form Data related to the dmp were created
        self.assertEqual(DmpFormData.objects.filter(dmp=dmp).count(), 2)

        # create another dmp (dmp form 2)
        dmp2 = self.create_dmp(
            self.token1, self.project.pk, "Second DMP", Dmp.NEW, self.dmp_form2.pk, HTTP_USER_AGENT, REMOTE_ADDR
        )
        # verify that a dmp was saved in the database
        self.assertEqual(Dmp.objects.all().count(), 2)
        self.assertEqual(dmp2.title, "Second DMP")
        self.assertEqual(dmp2.status, Dmp.NEW)

        # verify that three DMP Form Data related to the dmp were created
        self.assertEqual(DmpFormData.objects.filter(dmp=dmp2).count(), 3)

        # verify that there is a total of 5 dmp form data
        self.assertEqual(DmpFormData.objects.all().count(), 5)

    def test_create_dmp_without_project(self):
        """tries to create a DMP without project"""

        response = self.rest_create_dmp(
            self.token1, None, "First DMP", Dmp.NEW, self.dmp_form1.pk, HTTP_USER_AGENT, REMOTE_ADDR
        )
        ...
        # create a new dmp which is associated to dmp form 1
        dmp = self.create_dmp(self.token1, None, "First DMP", Dmp.NEW, self.dmp_form1.pk, HTTP_USER_AGENT, REMOTE_ADDR)
        self.assertEqual(dmp.title, "First DMP")
        self.assertEqual(dmp.status, Dmp.NEW)
    ...
    def test_create_dmp_with_wrong_parameter(self):
        ...
        response = self.client.post(
            "/api/dmps/",
            {"status": Dmp.NEW, "dmp_form": self.dmp_form1.pk},
        ...
        response = self.client.post(
            "/api/dmps/",
            {"title": "", "status": Dmp.NEW, "dmp_form": self.dmp_form1.pk},
            HTTP_USER_AGENT=HTTP_USER_AGENT,
            REMOTE_ADDR=REMOTE_ADDR,
        )
        ...
        response = self.client.post(
            "/api/dmps/",
            {"title": "new dmp", "dmp_form": self.dmp_form1.pk, "projects": [self.project.pk]},
            HTTP_USER_AGENT=HTTP_USER_AGENT,
            REMOTE_ADDR=REMOTE_ADDR,
        )
        ...
        response = self.client.post(
            "/api/dmps/",
            {"title": "new dmp", "status": "", "dmp_form": self.dmp_form1.pk, "projects": [self.project.pk]},
            HTTP_USER_AGENT=HTTP_USER_AGENT,
            REMOTE_ADDR=REMOTE_ADDR,
        )
        ...
        response = self.client.post(
            "/api/dmps/",
            {"title": "new dmp", "status": Dmp.NEW, "projects": [self.project.pk]},
            HTTP_USER_AGENT=HTTP_USER_AGENT,
            REMOTE_ADDR=REMOTE_ADDR,
        )
        ...
        response = self.client.post(
            "/api/dmps/",
            {"title": "new dmp", "status": Dmp.NEW, "dmp_form": "", "projects": [self.project.pk]},
            HTTP_USER_AGENT=HTTP_USER_AGENT,
            REMOTE_ADDR=REMOTE_ADDR,
        )
    ...
    def test_update_dmp_and_nested_dmp_form_data(self):
        ...
        # create a new dmp
        dmp_object = self.create_dmp(
            self.token1, self.project.pk, "New DMP", Dmp.NEW, self.dmp_form1.pk, HTTP_USER_AGENT, REMOTE_ADDR
        )
        ...
        data = {
            ...
            "dmp_form_data": [{"pk": str(dmp_form_data_object.pk), "value": "12345"}],
        }

        response = self.client.put(
            f"/api/dmps/{dmp_object.pk}/",
            json.dumps(data),
        ...
        self.assertEqual(
            decoded_response["dmp_form_data"][0]["value"],
            "12345",
    ...
    def test_update_dmp_with_wrong_parameter(self):
        ...
        dmp_object = self.create_dmp(
            self.token1, self.project.pk, "Update DMP", Dmp.NEW, self.dmp_form1.pk, HTTP_USER_AGENT, REMOTE_ADDR
        )
        # get first dmp form data object for the specific dmp object
        dmp_form_data_object = DmpFormData.objects.filter(dmp=dmp_object.pk).first()

        # create a new dmp with dmp form 2
        dmp_object_2 = self.create_dmp(
            self.token1, self.project.pk, "Update DMP 2", Dmp.NEW, self.dmp_form2.pk, HTTP_USER_AGENT, REMOTE_ADDR
        )
        # get first dmp form data object for the specific dmp object
        ...
        response = self.client.put(
            f"/api/dmps/{dmp_object.pk}/",
            {"status": Dmp.PROGRESS, "dmp_form": self.dmp_form1.pk},
            HTTP_USER_AGENT=HTTP_USER_AGENT,
            REMOTE_ADDR=REMOTE_ADDR,
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        ...
        response = self.client.put(
            f"/api/dmps/{dmp_object.pk}/",
            {"title": "update dmp", "dmp_form": self.dmp_form1.pk},
            HTTP_USER_AGENT=HTTP_USER_AGENT,
            REMOTE_ADDR=REMOTE_ADDR,
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        response = self.update_dmp(
            self.token1, self.project.pk, "update dmp", Dmp.PROGRESS, dmp_object.pk, "", HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.content.decode(), '{"dmp_form":["This field may not be null."]}')
        ...
        decoded_response = json.loads(response.content.decode())
        self.assertEqual(
            decoded_response["projects"][0], str(self.project.pk), msg="check if the project pk was not updated"
        )
        ...
        response = self.update_dmp_and_nested_dmp_form_data(
            self.token1,
            self.project.pk,
            "update dmp",
            Dmp.PROGRESS,
        ...
        response = self.update_dmp_and_nested_dmp_form_data(
            ...
            dmp_object.pk,
            self.dmp_form1.pk,
            second_dmp_form_data_object.pk,
            "12345",
            HTTP_USER_AGENT,
            REMOTE_ADDR,
        )
        ...
        data = {
            ...
            "status": Dmp.PROGRESS,
            "dmp_form": str(self.dmp_form1.pk),
            "dmp_form_data": [{"pk": str(dmp_form_data_object.pk)}],
        }
        response = self.client.put(
            f"/api/dmps/{dmp_object.pk}/",
            json.dumps(data),
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            response.content.decode(), '{"' + format(dmp_form_data_object.pk) + '":["The value has to be a number"]}'
        )
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            decoded_response["dmp_form_data"][1]["value"], "my new value", msg="value should have been updated"
        )
    ...
    def test_dmp_status_final_with_correct_user(self):
        ...
        # create a new dmp
        dmp_object = self.create_dmp(
            self.token1, self.project.pk, "New Final DMP", Dmp.FINAL, self.dmp_form1.pk, HTTP_USER_AGENT, REMOTE_ADDR
        )
        # get a dmp form data object for the specific dmp object
        dmp_form_data_object = DmpFormData.objects.filter(dmp=dmp_object.pk).first()
    ...
    def test_dmp_status_final_with_wrong_user(self):
        ...
        self.validate_assign_user_to_project(
            self.token1, self.project, self.user2, self.projectManagerRole, HTTP_USER_AGENT, REMOTE_ADDR
        )
