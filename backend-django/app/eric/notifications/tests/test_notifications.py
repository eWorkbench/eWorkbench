...
from eric.core.tests.test_utils import CommonTestMixin, FakeRequest, FakeRequestUser, naive_dt
...
from eric.shared_elements.models import CalendarAccess, Task
from eric.shared_elements.tests.core import MeetingMixin, TaskMixin

User = get_user_model()


class NotificationsTest(
    APITestCase, AuthenticationMixin, UserMixin, TaskMixin, MeetingMixin, NotificationMixIn, RelationsMixin
):
    def setUp(self):
    ...
    def test_verify_auto_created_notification_configuration(self):
        ...
        """
        self.assertEqual(
            NotificationConfiguration.objects.all().count(),
            2,
    ...
    def test_creating_notifications_task(self):
        ...
        self.assertEqual(Notification.objects.filter(user=self.user2, created_by=self.user1).count(), 2)
        self.assertEqual(
        ...
        self.assertEqual(Notification.objects.filter(user=self.user1, created_by=self.user2).count(), 1)
        ...
        response = self.rest_update_task_assigned_users(
            self.token1, str(task.pk), [self.user1.pk], HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        self.assertEqual(Notification.objects.filter(user=self.user2, created_by=self.user1).count(), 3)
    ...
    def test_creating_notifications_meeting(self):
        ...
        self.rest_update_meeting_attending_users(
            self.token1, str(meeting.pk), [self.user1.pk, self.user2.pk], HTTP_USER_AGENT, REMOTE_ADDR
        )
        ...
        response = self.rest_update_meeting_attending_users(
            self.token1, str(meeting.pk), [self.user1.pk], HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_read_one_notification(self):
        ...
        self.assertEqual(Notification.objects.filter(read=False).count(), 3)
        ...
        response = self.rest_mark_notification_as_read(
            self.token2, str(Notification.objects.filter(user=self.user2).first().pk), HTTP_USER_AGENT, REMOTE_ADDR
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        # now user2 should have one unread and one read notification
        self.assertEqual(Notification.objects.filter(read=False, user=self.user2).count(), 1)
        self.assertEqual(Notification.objects.filter(read=True, user=self.user2).count(), 1)

        # but user1 should still have one unread notification
        self.assertEqual(Notification.objects.filter(read=False, user=self.user1).count(), 1)
    ...
    def test_read_all_notifications(self):
        ...
        self.assertEqual(Notification.objects.filter(read=False, user=self.user2).count(), 0)
        self.assertEqual(Notification.objects.filter(read=True, user=self.user2).count(), 2)
...
class AppointmentConfirmationMailTest(
    ...
    def setUp(self):
        self.user_group = Group.objects.get(name="User")
        self.user1, self.token1 = self.create_user_and_log_in(username="user1", email="one@test.local", groups=["User"])
        self.user2, self.token2 = self.create_user_and_log_in(username="user2", email="two@test.local", groups=["User"])
    ...
    def test_confirmation_mail_content_with_resource_booking(self):
        ...
        resource, response = self.create_resource_orm(
            ...
            description="My Test Resource Description",
    ...
    def test_create_for_user_is_notified_too(self):
        ...
        with FakeRequest(), FakeRequestUser(self.user2):
            user2_calendar_access = ModelPrivilege.objects.get(
                content_type=CalendarAccess.get_content_type(), user=self.user2
            )
        ...
        self.assertEqual(
            NotificationConfiguration.NOTIFICATION_CONF_MEETING_USER_CHANGED, notification.notification_type
        )
...
class ScheduledNotificationsTest(
    ...
    def test_creating_scheduled_notifications_for_meeting(self):
        ...
        self.assertEqual(ScheduledNotification.objects.all().count(), 0)
        ...
        meeting, response = self.create_meeting_orm(
            ...
            scheduled_notification_writable={"active": True, "timedelta_unit": "MINUTE", "timedelta_value": 10},
            **HTTP_INFO,
        )
        self.assert_response_status(response, expected_status_code=status.HTTP_201_CREATED)

        # there should be one scheduled notification now
        self.assertEqual(ScheduledNotification.objects.all().count(), 1)
        ...
        with time_machine.travel(now_plus_15, tick=False):
            ...
            self.assertNotEqual(scheduled_notification_datetime, new_scheduled_notification_datetime)
        ...
        with time_machine.travel(now_plus_35, tick=False):
            ...
            self.assertEqual(second_new_scheduled_notification_datetime, new_scheduled_notification_datetime)


class TaskReminderNotificationsTest(
    APITestCase, CommonTestMixin, AuthenticationMixin, UserMixin, TaskMixin, NotificationMixIn
):
    def setUp(self):
        self.user_group = Group.objects.get(name="User")
        self.user1, self.token1 = self.create_user_and_log_in(username="user1", email="one@test.local", groups=["User"])
    ...
    def test_creating_reminder_notifications_for_tasks(self):
        ...
        with time_machine.travel(now_plus_20, tick=False):
            ...
            self.assertEqual(reminder_notifications.count(), 0)
