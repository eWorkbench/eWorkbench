...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        users_that_recently_received_notifications = Notification.objects.filter(
            processed=True, sent__gte=timezone.now() - MINIMUM_TIME_BETWEEN_EMAILS
        ).values("user__pk")

        # get all notifications by active users that have not been processed and not been sent yet
        notifications = (
            Notification.objects.filter(sent__isnull=True, processed=False, user__is_active=True)
            .exclude(
                Q(user__in=Subquery(users_that_recently_received_notifications))
                | Q(
                    user__last_login=None
                )  # Invited users who have not yet logged in should never receive notifications
            )
            .prefetch_related(
                "user", "user__userprofile", "user__notification_configuration", "created_by", "created_by__userprofile"
            )
        )
        ...
        for task in tasks:
            ...
            for assigned_user in assigned_users:
                Notification.objects.get_or_create(
                    ...
                    defaults={"title": title, "message": html_message, "created_at": now},
                )
    ...
    def process_scheduled_notifications(cls):
        ...
        for scheduled_notification in scheduled_notifications:
            ...
            if meeting.local_date_time_start > timezone.now():
                ...
                title = _(
                    "Reminder: Appointment {title} starts at {date_time_start}".format(
                        title=meeting.title, date_time_start=date_short(meeting.local_date_time_start)
                    )
                )
                ...
                for attending_user in attending_users:
                    Notification.objects.update_or_create(
                        ...
                        defaults={"title": title, "message": html_message, "created_at": timezone.now()},
                    )
    ...
    def aggregate_and_send_notifications(cls, notifications_qs, notification_count):
        ...
        if notification_count > 0:
            print(
                "[{datetime}] Processing {num_notifications} notifications for {num_users} users".format(
