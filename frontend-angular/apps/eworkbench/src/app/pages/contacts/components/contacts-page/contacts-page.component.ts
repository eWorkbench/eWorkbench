...
import { MergeDuplicatesModalComponent } from '../modals/merge-duplicates/merge-duplicates.component';
import { NewContactModalComponent } from '../modals/new/new.component';

@UntilDestroy()
@Component({
  selector: 'eworkbench-contacts-page',
...
export class ContactsPageComponent implements OnInit {
  ...
  @ViewChild('academicTitleCellTemplate', { static: true })
  public academicTitleCellTemplate!: TemplateRef<any>;

  @ViewChild('phoneCellTemplate', { static: true })
  public phoneCellTemplate!: TemplateRef<any>;

  @ViewChild('companyCellTemplate', { static: true })
  public companyCellTemplate!: TemplateRef<any>;
  ...
  public initTranslations(project = false): void {
    this.translocoService
      .selectTranslate('contacts.title')
    ...
    this.translocoService
      ...
      .subscribe(column => {
        ...
        if (this.currentUser?.userprofile.ui_settings?.tables?.contacts) {
          const merged = merge(
            keyBy(this.currentUser.userprofile.ui_settings.tables.contacts, 'key'),
            keyBy(
        ...
        if (this.currentUser?.userprofile.ui_settings?.tables_sort?.contacts) {
          this.sorting = this.currentUser.userprofile.ui_settings.tables_sort.contacts;
        }

        if (this.currentUser?.userprofile.ui_settings?.filter_settings?.contacts) {
          const filters = this.currentUser.userprofile.ui_settings?.filter_settings?.contacts;
