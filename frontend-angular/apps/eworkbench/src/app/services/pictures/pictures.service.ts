...
export class PicturesService
  ...
  public getList(params?: HttpParams): Observable<{ total: number; data: Picture[] }> {
    return this.httpClient
      .get<DjangoAPI<Picture[]>>(this.apiUrl, { params })
      .pipe(
        map(
  ...
  public add(picture: PicturePayload | SketchPayload, params?: HttpParams): Observable<Picture> {
  ...
  public get(id: string, userId: number, params?: HttpParams): Observable<PrivilegesData<Picture>> {
    return this.httpClient
      .get<Picture>(`${this.apiUrl}${id}/`, { params })
      .pipe(
        switchMap(
  ...
  public delete(id: string, params?: HttpParams): Observable<Picture> {
    return this.httpClient.patch<Picture>(`${this.apiUrl}${id}/soft_delete/`, { pk: id }, { params });
  }

  public patch(id: string, task: Optional<PicturePayload>, params?: HttpParams): Observable<Picture> {
    return this.httpClient.patch<Picture>(`${this.apiUrl}${id}/`, { pk: id, ...task }, { params });
  }

  public restore(id: string, params?: HttpParams): Observable<Picture> {
  ...
  public uploadImage(id: string, picture: PictureEditorPayload, params?: HttpParams): Observable<Picture> {
    const formData = new FormData();
    for (const [key, val] of Object.entries(picture)) {
      if (!val) continue;
      if (val instanceof Blob) {
  ...
  public downloadShapes(url: string, params?: HttpParams): Observable<any> {
    return this.httpClient.get<any>(url, { params });
  }

  public convertTiff(url: string, picture: ConvertTiffPayload, params?: HttpParams): Observable<Blob> {
    const formData = new FormData();
    for (const [key, val] of Object.entries(picture)) {
      if (!val) continue;
      if (val instanceof Blob) {
